...
#End If
    ...
    Private Function TandFXmlCleanup(ByVal XmlString As String) As String
        ...
        GBL.DeantaBallon($"Client type : {OrgDocType.ToString()}", MessageType.MSGINFO)
                ...
                Try
                    ...
                    If ((GraphicNodes IsNot Nothing) AndAlso (GraphicNodes.Count > 0)) Then
                        For c As Int16 = GraphicNodes.Count - 1 To 0 Step -1
                            sGraphic = Regex.Replace(GraphicNodes(c).Attributes("xlink:href").Value, "f(\d+)\.(\d+)", "fig$1_$2.tif", RegexOptions.IgnoreCase)
                ...
                Try
                    ...
                    Select Case token
                        ...
                        Case Else
                            ...
                            IsRemoveToken = False
                    End Select
                    Dim LstAttb As XmlAttribute = xmlFloat.CreateNode(XmlNodeType.Attribute, "list-type", "")
        ...
        If ((BookPartList IsNot Nothing) AndAlso (BookPartList.Count > 0)) Then
            For pt As Integer = 0 To BookPartList.Count - 1
                ...
                If ((XrefPageList IsNot Nothing) AndAlso (XrefPageList.Count > 0) AndAlso (BodyNode IsNot Nothing)) Then
                    ...
                    End Try
                    Try
                        Dim iMax As Int16 = (From n In XrefPageList Select n.Attributes("id").Value.Replace("page_", "")).Max
                ...
                For f As Int16 = 0 To Footnotes.Count - 1
                    Try
                        ...
                        If (Not String.IsNullOrEmpty(IbidMat)) Then
                            ...
                            Continue For
                        End If
                        IbidMat = (From n In IbidList Where n.Contains(IIf(Footnotes(f).InnerText.EndsWith("."), Footnotes(f).InnerText, $"{Footnotes(f).InnerText}.")) Select n).FirstOrDefault
                        If (Not String.IsNullOrEmpty(IbidMat)) Then
                            Footnotes(f).InnerXml = Footnotes(f).InnerXml.Replace(IbidMat, IbidText)
                            Continue For
                        End If
                        IbidMat = (From n In IbidList Where n.Contains(IIf(Footnotes(f).InnerText.EndsWith(";"), Footnotes(f).InnerText, $"{Footnotes(f).InnerText.Replace(".", "")};")) Select n).FirstOrDefault
                        If (Not String.IsNullOrEmpty(IbidMat)) Then
                            Footnotes(f).InnerXml = Footnotes(f).InnerXml.Replace(IbidMat, IbidText)
                            Continue For
                        End If
