...
#End If
        ...
        Try
            ...
            Using XMLWrite As StreamWriter = File.AppendText(Path.Combine(sXMLFilePath, sXMLFileName))
                For i = 0 To FileSequence.Count - 1
                    ...
                    'If (DocType = DocumentType.BLOOMSBURY) Then
                    ...
                    FootnoteType = False
                    XMLString = XMLString.Replace("<pasteboard><abstract><title>Abstract</title>", "<pasteboard><abstract>")
                    XMLString = XMLString.Replace("<!--INDRO<pasteboard>", "").Replace("</pasteboard>-->", "")
                    XMLString = XMLString.Replace("<title>Click here to enter text.</title>", "")
                    XMLString = XMLString.Replace("<emphasis role=""entity"">&amp;</emphasis>", "&#x0026;")
                    XMLString = Regex.Replace(XMLString, "<emphasis role=""italic""[\s]*/>", "", RegexOptions.Singleline Or RegexOptions.IgnoreCase)
                    XMLString = XMLString.Replace("<emphasis>&#x2013;</emphasis>", "&#x2013;")
                    XMLString = XMLString.Replace("<superscript>&#x2013;</superscript>", "&#x2013;")
                    XMLString = XMLString.Replace("<link role=""page""> </link>", "")
                    XMLString = XMLString.Replace("cols=""0""", "")
                    XMLString = XMLString.Replace(" role=""noindent""", "")
                    ...
                    If ((DocType = DocumentType.BLOOMSBURY) Or (DocType = DocumentType.TRD)) Then
                        XMLString = XMLString.Replace("</speaker>&#x2003;", "&#x2003;</speaker>")
                    End If

                    XMLString = Regex.Replace(XMLString, $"(<title>(?:(?:(?!<\/title>).)*)</title>)(<index>)", "$2$1", RegexOptions.Singleline Or RegexOptions.IgnoreCase)

                    If (Regex.Match(XMLString, "(<chapter [^>]*>)(<index[^>]*>)").Success And (XMLString.Contains("</index></chapter>"))) Then
                        XMLString = Regex.Replace(XMLString, $"(<chapter [^>]*>)(<index[^>]*>)", "$2", RegexOptions.Singleline Or RegexOptions.IgnoreCase).Replace("</index></chapter>", "</index>")
                    End If

                    If (Not IsePubXML) Then
                        XMLString = Regex.Replace(XMLString, "</epigraph><epigraph[^>]*>", "", RegexOptions.Singleline Or RegexOptions.IgnoreCase)
                    End If
                    ...
                    If ((IsBookEndNote) And (Not IsePubXML)) Then
                        'If (DocType = DocumentType.BLOOMSBURY) Then
                    ...
                    Dim fnChapNum As String = String.Empty
                    fnChapNum = Regex.Match(Path.GetFileNameWithoutExtension(FileSequence(i)), "_C([0-9]+)_", RegexOptions.Singleline Or RegexOptions.IgnoreCase).Groups(1).Value
                    If (fnChapNum = "") Then
                        fnChapNum = FnChapterNum
                        FnChapterNum = FnChapterNum + 1
                    End If

                    XMLString = Regex.Replace(XMLString, "<xref [^>]*>", Function(fMt As Match)
                                                                             Return Regex.Replace(fMt.Value, "rid=""fn", "rid=""ch" & fnChapNum.TrimStart("0") & "-fn", RegexOptions.IgnoreCase)
                                                                         End Function, RegexOptions.Singleline Or RegexOptions.IgnoreCase)
                    ...
                    If ((Me.DocType = DocumentType.BLOOMSBURY) Or (Me.DocType = DocumentType.TRD) Or (Me.DocType = DocumentType.RL) Or (Me.DocType = DocumentType.SEQUOIA)) Then
                        XMLString = XMLString.Replace("end-ch-note", "end-bk-note")
                    End If
                    ...
                    XMLString = Regex.Replace(XMLString, "(<sup>((?:(?!<\/sup>).)*)</sup></xref>)(<footnote [^>]*>)", Function(m As Match)
                                                                                                                          ...
                                                                                                                          Else
                                                                                                                              sInput = sInput.TrimEnd(">") & " role=" & Chr(34) & RoleValue & Chr(34) & " label=" & Chr(34) & "ch" & fnChapNum.TrimStart("0", "") & "-fn" & smt.Groups(1).Value.ToString & Chr(34) & " xml:id=""note" & iFootnote & "-ba-" & String.Format("{0:00000}", iFootnote) & """>"
                    ...
                    End If
                    If ((Me.DocType = DocumentType.BLOOMSBURY) Or (Me.DocType = DocumentType.TRD)) Then
                        XMLString = XMLString.Replace("role=""end-bk-note""", "role=""end-ch-note""")
                    End If
                        ...
                        Try
                            If ((XMLString.Contains("</chapter>") Or XMLString.Contains("</acknowledgements>") Or XMLString.Contains("</preface>")) And (Not XMLString.Contains("<index>"))) Then
                                ...
                                If ((FootList IsNot Nothing) AndAlso (FootList.Count > 0)) Then
                                    ...
                                    If ((FootList IsNot Nothing) AndAlso (FootList.Count > 1)) Then
                                    ...
                                    Else
                                        XMLString = Regex.Replace(XMLString, "</chapter>", "<fn-group><title>Note</title></chapter>", RegexOptions.IgnoreCase Or RegexOptions.Singleline)
                                        XMLString = Regex.Replace(XMLString, "</preface>", "<fn-group><title>Note</title></preface>", RegexOptions.IgnoreCase Or RegexOptions.Singleline)
                                        XMLString = Regex.Replace(XMLString, "</acknowledgements>", "<fn-group><title>Note</title></acknowledgements>", RegexOptions.IgnoreCase Or RegexOptions.Singleline)
                                    End If
                ...
                If (bxslExecution) Then
                    If ((OrgDocType = DocumentType.RL) Or (OrgDocType = DocumentType.SEQUOIA)) Then
                        sXMLTxt = Regex.Replace(sXMLTxt, "<equation[^><]+>((?:(?!</equation>).)+)</equation>", AddressOf MathCleanUpforRL, RegexOptions.IgnoreCase Or RegexOptions.Singleline)
                    Else
            ...
            If Not bxslExecution Then
                ...
                XMLString = XMLString.Replace("</nav-pointer-group>, <nav-pointer-group>", ", ").Replace("</nav-pointer-group><bold>&#x2013;</bold><nav-pointer-group>", "&#x2013;")
                XMLString = XMLString.Replace("</nav-pointer-group>, <nav-pointer-group>", ", ").Replace("</nav-pointer-group><italic>&#x2013;</italic><nav-pointer-group>", "&#x2013;")
                ...
                'merge imprint-text ' 30-12-2021
                XMLString = Regex.Replace(XMLString, "<imprint-text>Names: (((?!<imprint-text>Title:).)*)</imprint-text>", Function(mt As Match)
                                                                                                                               Return mt.Value.Replace("</imprint-text><imprint-text>", "")
                                                                                                                           End Function, RegexOptions.Singleline Or RegexOptions.IgnoreCase)

                XMLString = Regex.Replace(XMLString, "<imprint-text>Title: (((?!<imprint-text>Description:).)*)</imprint-text>", Function(mt As Match)
                                                                                                                                     Return mt.Value.Replace("</imprint-text><imprint-text>", "")
                                                                                                                                 End Function, RegexOptions.Singleline Or RegexOptions.IgnoreCase)

                XMLString = Regex.Replace(XMLString, "<imprint-text>Description: (((?!<imprint-text>Identifiers:).)*)</imprint-text>", Function(mt As Match)
                                                                                                                                           Return mt.Value.Replace("</imprint-text><imprint-text>", "")
                                                                                                                                       End Function, RegexOptions.Singleline Or RegexOptions.IgnoreCase)

                XMLString = Regex.Replace(XMLString, "<imprint-text>Identifiers: (((?!<imprint-text>Subjects:).)*)</imprint-text>", Function(mt As Match)
                                                                                                                                        Return mt.Value.Replace("</imprint-text><imprint-text>", "")
                                                                                                                                    End Function, RegexOptions.Singleline Or RegexOptions.IgnoreCase)

                XMLString = Regex.Replace(XMLString, "<imprint-text>Subjects: (((?!<imprint-text>Classification:).)*)</imprint-text>", Function(mt As Match)
                                                                                                                                           Return mt.Value.Replace("</imprint-text><imprint-text>", "")
                                                                                                                                       End Function, RegexOptions.Singleline Or RegexOptions.IgnoreCase)
                XMLString = Regex.Replace(XMLString, "<imprint-text>Classification: (((?!<imprint-text>LC record).)*)</imprint-text>", Function(mt As Match)
                                                                                                                                           Return mt.Value.Replace("</imprint-text><imprint-text>", "")
                                                                                                                                       End Function, RegexOptions.Singleline Or RegexOptions.IgnoreCase)

                XMLString = XMLString.Replace("><", $">{vbCrLf}<")

                Dim RunnoList As New List(Of String)
                RunnoList.AddRange(New String() {"italic", "bold"})
                For r As Int16 = 0 To RunnoList.Count - 1
                    XMLString = XMLString.Replace($"{vbCrLf}<{RunnoList(r)}>", $"<{RunnoList(r)}>").Replace($"</{RunnoList(r)}>{vbCrLf}", $"</{RunnoList(r)}>")
                Next


                XMLString = XMLString.Replace("<contrib content-type", "<contrib contrib-type")
                XMLString = XMLString.Replace("<p> ", "<p>").Replace($"<list-item>{vbCrLf}<p>", "<list-item><p>").Replace($"</p>{vbCrLf}</list-item>", "</p></list-item>")
                XMLString = XMLString.Replace($"<p>{vbCrLf}<!--<token>-->", "<p><!--<token>-->")
                XMLString = XMLString.Replace($"<entry>{vbCrLf}<p>", "<entry><p>").Replace($"</p>{vbCrLf}</entry>", "</p></entry>")
                XMLString = Regex.Replace(XMLString, $"(<list [^>]*>){vbCrLf}(<list-item>)", "$1$2", RegexOptions.Singleline Or RegexOptions.IgnoreCase)
                XMLString = Regex.Replace(XMLString, $"(<entry>){vbCrLf}(<p[\s]*/>){vbCrLf}", "$1$2", RegexOptions.Singleline Or RegexOptions.IgnoreCase)
                XMLString = XMLString.Replace($"<title>{vbCrLf}<xref", "<title><xref")
                XMLString = XMLString.Replace("</nav-pointer-group>;<index-", ";</nav-pointer-group><index-")
            ...
            Else
                ...
                If (String.IsNullOrEmpty(sXMLTxt)) Then
                    Return False
                End If
                sXMLTxt = Regex.Replace(sXMLTxt, "(<book [^>]*)>", "$1 client=""" & OrgDocType.ToString() & """>", RegexOptions.Singleline Or RegexOptions.IgnoreCase)
    ...
    Private Function RemoveEmptyElements(ByVal Contents As String) As String
        ...
        EmptyElementList.AddRange(New String() {"issuenum", "surname", "given-names", "firstname", "publisher-loc", "address", "source", "chapter-title", "title", "collab", "chapter-title", "issue", "volume", "token", "publishername", "remark", "tp"})
    ...
    Private Function AddMetadataInformation(ByVal FinalXML As String) As String
        ...
        TNFMeta.Add(New TNFMetaDataCollector With {.ElementName = "epubisbn", .ElementType = TNFMetaDataType.PublisherName, .MetaSource = MetadataType.DB, .ElementDestXPath = "./isbn[@pub-type='epub']", .ElementSourceXPath = "", .DBColumnName = "epub_isbn"})
        TNFMeta.Add(New TNFMetaDataCollector With {.ElementName = "webisbn", .ElementType = TNFMetaDataType.PublisherName, .MetaSource = MetadataType.DB, .ElementDestXPath = "./isbn[@pub-type='web']", .ElementSourceXPath = "", .DBColumnName = "ebook_pdf"})
        ...
        Try
            ...
            FinalXML = FinalXML.Replace("&", "&amp;")
            FinalXML = FinalXML.Replace("</imprint-text>", "</imprint-text></imprint-meta>").Replace("<imprint-text", "<imprint-meta><imprint-text")
            FinalXML = FinalXML.Replace("</imprint-meta><imprint-meta>", "")
        ...
        Try
            xmlMeta.MoveXMLNode("//imprint-text[@type='ImprintStatement']", "//publisher", NodeMoveOption.MOVEAFTER)
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        End Try
        ...
        Try
            RemoveParaAttributes()
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message & "RemoveParaAttributes", MessageType.MSGERROR)
        End Try

        Try
            FormatMetadata()
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message & "FormatMetadata", MessageType.MSGERROR)
        End Try

        Try
            IndexandReference()
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message & "IndexandReference", MessageType.MSGERROR)
        End Try

        Dim FinalXMLCont As String = xmlMeta.OuterXml.Replace("&amp;", "&")
        FinalXMLCont = Regex.Replace(FinalXMLCont, $"</back>[\s{vbCrLf}]+<back>", "", RegexOptions.Singleline Or RegexOptions.IgnoreCase)
        Dim SelfCloseTags As New List(Of String)
        SelfCloseTags.AddRange(New String() {"italic", "bold", "strike", "token", "article-title", "volume", "issue", "body", "surname", "given-names", "collab", "publisher-loc", "publisher-name", "source", "title", "chapter-title", "string-name"})
        Try
            FinalXMLCont = FinalXMLCont.Replace("..</given-name>", ".</given-name>")
            FinalXMLCont = FinalXMLCont.Replace(",.</given-name>", ".</given-name>")
            FinalXMLCont = FinalXMLCont.Replace(".,.</given-name>", ".</given-name>")
            FinalXMLCont = FinalXMLCont.Replace(".,</given-name>", ".</given-name>")
            FinalXMLCont = FinalXMLCont.Replace(" </given-name>", "</given-name>")
            FinalXMLCont = FinalXMLCont.Replace(".</label>", "</label>")
            FinalXMLCont = FinalXMLCont.Replace(",</surname>", "</surname>,")
            FinalXMLCont = FinalXMLCont.Replace(" </surname>", "</surname>,")
            FinalXMLCont = FinalXMLCont.Replace("<italic> </italic>", " ")
            FinalXMLCont = FinalXMLCont.Replace("<bold> </bold>", " ")
            FinalXMLCont = FinalXMLCont.Replace("<honorific>", "<suffix>").Replace("</honorific>", "</suffix>")
            FinalXMLCont = FinalXMLCont.Replace("<poem>", "<disp-quote>").Replace("</poem>", "</disp-quote>")
            FinalXMLCont = FinalXMLCont.Replace("<poemsource>", "<attrib>").Replace("</poemsource>", "</attrib>")
            FinalXMLCont = FinalXMLCont.Replace("<tblfoot>", "").Replace("</tblfoot>", "")
            FinalXMLCont = FinalXMLCont.Replace("&apos;", "")
            FinalXMLCont = FinalXMLCont.Replace("<tblsource>", "<attrib>").Replace("</tblsource>", "</attrib>")
            FinalXMLCont = FinalXMLCont.Replace("<p> ", "<p>")
            FinalXMLCont = FinalXMLCont.Replace("</surname>,</name>", "</surname></name>")
            FinalXMLCont = Regex.Replace(FinalXMLCont, " (<p[\s]+/>)", "$1", RegexOptions.Singleline Or RegexOptions.IgnoreCase)
            FinalXMLCont = FinalXMLCont.Replace("<kwd-group>", "<kwd-group kwd-group-type=""catchwords"">")
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        End Try

        Try
            For s As Int16 = 0 To SelfCloseTags.Count - 1
                FinalXMLCont = Regex.Replace(FinalXMLCont, $"<{SelfCloseTags(s)}[\s]*/>", "", RegexOptions.Singleline Or RegexOptions.IgnoreCase)
            Next
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        End Try
    ...
    Private Function IndexandReference() As Boolean
        Try
            xmlMeta.RemoveAttribute("//index-entry[@display='no']", "display")
            xmlMeta.RemoveXMLNodeOnly("//term/term")
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        End Try

        Dim TermNodes As XmlNodeList = xmlMeta.SelectNodes("//term")
        If ((TermNodes IsNot Nothing) AndAlso (TermNodes.Count > 0)) Then
            For t As Int16 = 0 To TermNodes.Count - 1
                If (String.IsNullOrEmpty(TermNodes(t).InnerXml.Trim())) Then
                    If (TermNodes(t).ParentNode IsNot Nothing) Then
                        TermNodes(t).ParentNode.RemoveChild(TermNodes(t))
                        Continue For
                    End If
                End If
                TermNodes(t).InnerXml = TermNodes(t).InnerXml.Trim().TrimEnd(" ").TrimStart(" ")
            Next
        End If
        Return True
    End Function
    Private Function FormatMetadata() As Boolean
        Try
            xmlMeta.RemoveXMLNode("//series", NodeMoveOption.EMPTYTEXT)
            xmlMeta.RemoveXMLNode("//edition", NodeMoveOption.EMPTYTEXT)
            xmlMeta.RemoveXMLNode("//impression", NodeMoveOption.EMPTYTEXT)
            xmlMeta.RemoveXMLNode("//pub-date[child::year]", NodeMoveOption.NONE)
            xmlMeta.RemoveXMLNode("//permissions[child::copyright-statement]", NodeMoveOption.TEXTCONTAINS, "by Routledge")
            xmlMeta.RemoveXMLNode("//book-meta/p", NodeMoveOption.ENDSWITH, "edition")
            xmlMeta.RemoveXMLNode("//imprint-text[@type='BritishLibrary']", NodeMoveOption.TEXTCONTAINS, "British Library Cataloguing-in-Publication Data")
            xmlMeta.RemoveXMLNode("//imprint-text", NodeMoveOption.TEXTCONTAINS, "Library of Congress Cataloging-in-Publication Data")
            xmlMeta.RemoveXMLNode("//imprint-text", NodeMoveOption.TEXTCONTAINS, "Library of Congress Cataloging&#x2011;in&#x2011;Publication Data")
            xmlMeta.RemoveXMLNode("//abstract/p", NodeMoveOption.EMPTYTEXT)
            xmlMeta.RemoveXMLNode("//book-meta/p", NodeMoveOption.TEXTCONTAINS, "DOI: 10")
            xmlMeta.RemoveXMLNode("//book-meta/p", NodeMoveOption.TEXTCONTAINS, "Typeset in")
            xmlMeta.RemoveXMLNode("//book-meta/p", NodeMoveOption.TEXTCONTAINS, "by Deanta Global Publishing Services")
            xmlMeta.RemoveXMLNode("//book-meta/book-title-group[following-sibling::contrib-group[@content-type='author']]")
            xmlMeta.RemoveXMLNode("//book-meta/p[preceding-sibling::contrib-group and child::inline-graphic and count(child::node())=1]")
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        End Try
        Try
            xmlMeta.MoveXMLNode("//permissions", "//isbn[@pub-type='web']", NodeMoveOption.MOVEAFTER)
            xmlMeta.MoveXMLNode("//imprint-text[@type='ImprintStatement']", "//imprint-meta", NodeMoveOption.FIRSTCHILD)
            xmlMeta.RemoveXMLNodeOnly("//book-title/bold")
            xmlMeta.AddXMLChildNode("//permissions", "pub-date", $"<year>{xmlMeta.SelectSingleNode("//permissions/copyright-year").InnerText}</year>", NodeMoveOption.MOVEBEFORE)
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        End Try

        Try
            xmlMeta.MoveXMLNode("//ack[parent::book]", "//book-front", NodeMoveOption.LASTCHILD)
            xmlMeta.MoveXMLNode("//glossary[parent::book]", "//book-front", NodeMoveOption.LASTCHILD)
            xmlMeta.MoveXMLNode("//toc[@content-type='law-table'][parent::book]", "//book-front", NodeMoveOption.LASTCHILD)
            xmlMeta.MoveXMLNode("//toc[@content-type='law-table'][parent::book]", "//book-front", NodeMoveOption.LASTCHILD)
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        End Try

        Dim Glossary As XmlNode = xmlMeta.SelectSingleNode("//glossary[parent::book-front]")
        If ((Glossary IsNot Nothing) AndAlso (Glossary.Attributes IsNot Nothing) AndAlso (Glossary.Attributes.Count > 0)) Then
            Glossary.Attributes.RemoveAll()
        End If

        Try
            xmlMeta.MoveXMLNode("//abstract[@abstract-type='blurb']", "//imprint-meta", NodeMoveOption.MOVEAFTER)
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        End Try
        Try
            xmlMeta.MoveXMLNode("//kwd-group", "//abstract[@abstract-type='blurb']", NodeMoveOption.MOVEAFTER)
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        End Try

        Dim TitleGroups As XmlNodeList = xmlMeta.SelectNodes("//book-title-group")
        If ((TitleGroups IsNot Nothing) AndAlso (TitleGroups.Count > 0)) Then
            If (TitleGroups.Count = 2) Then
                If (TitleGroups(0).ChildNodes.Count < TitleGroups(1).ChildNodes.Count) Then
                    If (TitleGroups(0).ParentNode IsNot Nothing) Then
                        TitleGroups(0).ParentNode.RemoveChild(TitleGroups(0))
                    End If
                ElseIf (TitleGroups(0).ChildNodes.Count > TitleGroups(1).ChildNodes.Count) Then
                    If (TitleGroups(1).ParentNode IsNot Nothing) Then
                        TitleGroups(1).ParentNode.RemoveChild(TitleGroups(1))
                    End If
                End If
            End If
        End If

        Dim Abstracts As XmlNodeList = xmlMeta.SelectNodes("//book-part-meta/abstract[@abstract-type='abstract']")
        If (Abstracts IsNot Nothing) AndAlso (Abstracts.Count - 1) Then
            For a As Int16 = 0 To Abstracts.Count - 1
                Try
                    Abstracts(a).ParentNode.InnerXml = Abstracts(a).ParentNode.InnerXml.Replace(Abstracts(a).OuterXml, "") & Abstracts(a).OuterXml
                Catch ex As Exception
                    GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
                End Try
            Next
        ...
        Dim Contrib As XmlNodeList = xmlMeta.SelectNodes("//contrib-group/text()")
        If ((Contrib IsNot Nothing) AndAlso (Contrib.Count > 0)) Then
            For e As Int16 = 0 To Contrib.Count - 1
                If ((String.Compare(Contrib(e).InnerText.Trim(), "and", True) = 0) Or (String.Compare(Contrib(e).InnerText.Trim(), ",", True) = 0)) Then
                    If (Contrib(e).ParentNode IsNot Nothing) Then
                        Contrib(e).ParentNode.RemoveChild(Contrib(e))
                    End If
                End If
            Next
        End If
        ...
        Try
            xmlMeta.MoveXMLNode("//book-title-group", "//book-id[@pub-id-type='doi']", NodeMoveOption.MOVEAFTER)
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        End Try

        Dim Contribs As XmlNodeList = xmlMeta.SelectNodes("//contrib[ancestor::book-meta]")
        Dim BioNode As XmlNode = Nothing
        If ((Contribs IsNot Nothing) AndAlso (Contribs.Count > 0)) Then
            For c As Int16 = 0 To Contribs.Count - 1
                Dim Author As String = Contribs(c).InnerText
                BioNode = Nothing
                If (String.IsNullOrEmpty(Author)) Then
                    Continue For
                End If
                Dim BioNodes As XmlNodeList = xmlMeta.SelectNodes("//bio[@id]")
                If ((BioNodes IsNot Nothing) AndAlso (BioNodes.Count > 0)) Then
                    For b As Int16 = 0 To BioNodes.Count - 1
                        If (BioNodes(b).InnerText.Contains(Author)) Then
                            BioNode = BioNodes(b)
                            Exit For
                        End If
                    Next
                End If
                If (BioNode Is Nothing) Then
                    Continue For
                End If
                'Contribs(c).AppendChild(BioNode)
            Next
        End If

        Dim ImprintNode As XmlNodeList = xmlMeta.SelectNodes("//imprint-meta/publisher-name")
        Dim ImprintMeta As XmlNode = Nothing
        If ((ImprintNode IsNot Nothing) AndAlso (ImprintNode.Count > 0)) Then
            For m As Int16 = 0 To ImprintNode.Count - 1
                Dim Imprint As XmlNode = xmlMeta.CreateNode(XmlNodeType.Element, "imprint-text", "")
                Dim ImpAtt As XmlAttribute = xmlMeta.CreateNode(XmlNodeType.Attribute, "type", "")
                ImpAtt.Value = "ImprintStatement"
                Imprint.Attributes.Prepend(ImpAtt)
                Imprint.InnerXml = ImprintNode(m).InnerXml.Replace("<italic>", "").Replace("</italic>", "")
                ImprintMeta = xmlMeta.SelectSingleNode("//imprint-meta")
                If (ImprintNode(m).ParentNode IsNot Nothing) Then
                    ImprintNode(m).ParentNode.InsertBefore(Imprint, ImprintNode(m))
                    ImprintNode(m).ParentNode.RemoveChild(ImprintNode(m))
                End If
            Next
        End If
        ImprintNode = xmlMeta.SelectNodes("//imprint-text[not(@type)]")
        If ((ImprintNode IsNot Nothing) AndAlso (ImprintNode.Count > 0)) Then
            For m As Int16 = 0 To ImprintNode.Count - 1
                If (ImprintNode(m).InnerText.StartsWith("The right of")) Then
                    Dim TypeAtt As XmlAttribute = xmlMeta.CreateNode(XmlNodeType.Attribute, "type", "")
                    TypeAtt.Value = "ImprintMoralRights"
                    ImprintNode(m).Attributes.Prepend(TypeAtt)
                End If
            Next
        End If

        'order isbn elements.
        Dim Isbns As XmlNodeList = xmlMeta.SelectNodes("//book-meta/isbn")
        If ((Isbns IsNot Nothing) AndAlso (Isbns.Count > 0)) Then
            If (String.Compare(Isbns(0).Attributes("pub-type").Value, "ebk", True) <> 0) Then
                Dim EbkIsbn As XmlNode = xmlMeta.SelectSingleNode("//isbn[@pub-type='ebk']")
                If ((EbkIsbn IsNot Nothing) AndAlso (Isbns(0).ParentNode IsNot Nothing)) Then
                    Isbns(0).ParentNode.InsertBefore(EbkIsbn, Isbns(0))
                End If
            End If
        End If

        Dim DuplicatePages As XmlNodeList = xmlMeta.SelectNodes("//book-part-meta[count(descendant::xref[@ref-type='page'])>1]")
        If ((DuplicatePages IsNot Nothing) AndAlso (DuplicatePages.Count > 0)) Then
            For d As Int16 = 0 To DuplicatePages.Count - 1
                Dim PageNode As XmlNodeList = DuplicatePages(d).SelectNodes(".//descendant::xref[@ref-type='page']")
                Dim FirstPara As XmlNode = DuplicatePages(d).ParentNode.SelectSingleNode(".//body//p[1]")
                If (FirstPara IsNot Nothing) Then
                    FirstPara.PrependChild(PageNode(PageNode.Count - 1))
                End If
            Next
        End If

        Dim AckSections As XmlNodeList = xmlMeta.SelectNodes("//sec/title[text()='Acknowledgments']|//sec/title[text()='Acknowledegments']")
        If ((AckSections IsNot Nothing) AndAlso (AckSections.Count > 0)) Then
            For a As Int16 = 0 To AckSections.Count - 1
                If ((AckSections(a).ParentNode IsNot Nothing) AndAlso (AckSections(a).ParentNode.ParentNode IsNot Nothing)) Then
                    AckSections(a).ParentNode.ParentNode.InnerXml = AckSections(a).ParentNode.ParentNode.InnerXml.Replace(AckSections(a).ParentNode.OuterXml, $"<ack>{AckSections(a).ParentNode.InnerXml}</ack>")
                End If
            Next
        End If

        Dim AckNodes As XmlNodeList = xmlMeta.SelectNodes("//body")
        If ((AckNodes IsNot Nothing) AndAlso (AckNodes.Count > 0)) Then
            For a As Int16 = 0 To AckNodes.Count - 1
                Try
                    If ((AckNodes(a).LastChild IsNot Nothing) AndAlso (AckNodes(a).LastChild.Name = "ack")) Then
                        If ((AckNodes(a).ParentNode IsNot Nothing) AndAlso (AckNodes(a).ParentNode.Name = "book-part")) Then
                            Dim backNode As XmlNode = AckNodes(a).ParentNode.SelectSingleNode(".//back")
                            If (backNode IsNot Nothing) Then
                                backNode.PrependChild(AckNodes(a).LastChild)
                            End If
                        End If
                    End If
                Catch ex As Exception
        ...
        Dim tocNodes As XmlNodeList = xmlMeta.SelectNodes("//toc[@content-type='law-table']")
        If ((tocNodes IsNot Nothing) AndAlso (tocNodes.Count > 0)) Then
            For t As Int16 = 0 To tocNodes.Count - 1
                Dim IdAtt As XmlAttribute = xmlMeta.CreateNode(XmlNodeType.Attribute, "id", "")
                IdAtt.Value = $"toc{t + 1}"
                tocNodes(t).Attributes.Append(IdAtt)
            Next
        End If

        Try
            xmlMeta.AddXMLChildNode("//book-meta", "notes", $"<object-id pub-id-type=""doi"">10.4324/{sISBN}-fm</object-id>", NodeMoveOption.MOVEAFTER)
            xmlMeta.AddXMLAttribute("//notes[child::object-id[@pub-id-type]]", "notes-type", "metadata")
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        End Try

        Try
            xmlMeta.AddXMLChildNode("//book/back", "notes", $"<object-id pub-id-type=""doi"">10.4324/{sISBN}-fm</object-id>", NodeMoveOption.FIRSTCHILD)
            xmlMeta.AddXMLAttribute("//back/notes[child::object-id[@pub-id-type]]", "notes-type", "metadata")
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        End Try
    ...
    Private Function RemoveParaAttributes() As Boolean
        Dim ParaNodelist As XmlNodeList
        Dim Bioid As Int16 = 1
        Dim MaxBio As Int16 = 0
        ParaNodelist = xmlMeta.SelectNodes("//p[@role='author_bio']")
        If ((ParaNodelist IsNot Nothing) AndAlso (ParaNodelist.Count > 0)) Then
            MaxBio = ParaNodelist.Count * 2
            While ((ParaNodelist IsNot Nothing) AndAlso (ParaNodelist.Count > 0))
                Try
                    If (MaxBio = 0) Then
                        Exit While
                    End If
                    If (ParaNodelist(0).ParentNode IsNot Nothing) Then
                        Dim firstBoldNode As XmlNode = ParaNodelist(0).SelectSingleNode(".//bold")
                        If (firstBoldNode IsNot Nothing) Then
                            ParaNodelist(0).ParentNode.InnerXml = ParaNodelist(0).ParentNode.InnerXml.Replace(ParaNodelist(0).OuterXml, $"<bio id=""bio{Bioid}""><p>{ParaNodelist(0).InnerXml.Replace(firstBoldNode.OuterXml, $"<named-content content-type=""bio-name"">{firstBoldNode.InnerXml}</named-content>")}</p></bio>")
                        End If
                        'If (ParaNodelist(0).InnerXml.Contains(" < bold")) Then
                        '    ParaNodelist(0).ParentNode.InnerXml = ParaNodelist(0).ParentNode.InnerXml.Replace(ParaNodelist(0).OuterXml, $"<bio id=""bio{Bioid}"">{ParaNodelist(0).InnerXml.Replace("<bold>", "<p><named-content content-type=""bio-name"">").Replace("</bold>", "</named-content>")}</p></bio>")
                        'End If
                        Bioid = Bioid + 1
                    End If
                    ParaNodelist = xmlMeta.SelectNodes("//p[@role='author_bio']")
            ...
            End While
        End If
        ParaNodelist = xmlMeta.SelectNodes("//bio")
        Dim IdNode As XmlAttribute = Nothing
        If ((ParaNodelist IsNot Nothing) AndAlso (ParaNodelist.Count > 0)) Then
            For i As Int16 = 0 To ParaNodelist.Count - 1
                Try
                    IdNode = ParaNodelist(i).Attributes("id")
                Catch ex As Exception
                    IdNode = xmlMeta.CreateNode(XmlNodeType.Attribute, "id", "")
                    ParaNodelist(i).Attributes.Append(IdNode)
                End Try
                IdNode.Value = $"bio{i + 1}"
            Next
        End If

        ParaNodelist = xmlMeta.SelectNodes("//p[@*]")
        If ((ParaNodelist IsNot Nothing) AndAlso (ParaNodelist.Count > 0)) Then
            For p As Int16 = 0 To ParaNodelist.Count - 1
                If ((ParaNodelist(p).Attributes IsNot Nothing) AndAlso (ParaNodelist(p).Attributes.Count > 0)) Then
                    ParaNodelist(p).Attributes.RemoveAll()
                End If
            Next
        End If

        Dim ContribNode As XmlNode = xmlMeta.SelectSingleNode("//book-meta/contrib-group")
        If ((ContribNode IsNot Nothing) AndAlso (ContribNode.ParentNode IsNot Nothing)) Then
            ContribNode.ParentNode.InnerXml = ContribNode.ParentNode.InnerXml.Replace(ContribNode.OuterXml, $"{ContribNode.OuterXml}<pub-date><year>{Date.Now.Year}</year></pub-date>")
        End If



        'no need from here, its needs for bits - 08-03-2021
        'Dim BookMetaNode As XmlNode = xmlMeta.SelectSingleNode("//book-meta")
        'Dim doiNode As XmlNode = xmlMeta.SelectSingleNode("//book-id[@pub-id-type='doi']")
        'If ((BookMetaNode IsNot Nothing) AndAlso (BookMetaNode.ParentNode IsNot Nothing)) Then
        '    If (doiNode IsNot Nothing) Then
        '        BookMetaNode.ParentNode.InnerXml = BookMetaNode.ParentNode.InnerXml.Replace(BookMetaNode.OuterXml, $"{BookMetaNode.OuterXml}<front-matter><notes notes-type=""metadata""><object-id pub-id-type=""doi"">{doiNode.InnerText}-fm</object-id></notes></front-matter>")
        '    End If
        'End If

        Dim TblWraps As XmlNodeList = xmlMeta.SelectNodes("//table-wrap")
        If ((TblWraps IsNot Nothing) AndAlso (TblWraps.Count > 0)) Then
            For t As Int16 = 0 To TblWraps.Count - 1
                Dim tblSource As XmlNode = TblWraps(t).SelectSingleNode(".//tblsource")
                Dim tblWrapFoot As XmlNode = TblWraps(t).SelectSingleNode(".//table-wrap-foot")
                If ((tblSource IsNot Nothing) AndAlso (tblWrapFoot IsNot Nothing)) Then
                    tblWrapFoot.AppendChild(tblSource)
                End If
            Next
        End If

        Dim ImprintTextNodes As XmlNodeList = xmlMeta.SelectNodes("//imprint-text")
        If ((ImprintTextNodes IsNot Nothing) AndAlso (ImprintTextNodes.Count > 0)) Then
            For m As Int16 = 0 To ImprintTextNodes.Count - 1
                ImprintTextNodes(m).InnerXml = ImprintTextNodes(m).InnerXml.Replace("LC</uri> ", "</uri>LC ").Replace("LC""", """")
            Next
        End If

        Dim SeeAlsoNodes As XmlNodeList = xmlMeta.SelectNodes("//index-entry//italic")
        Dim MdxCnt As Int16 = 0
        If ((SeeAlsoNodes IsNot Nothing) AndAlso (SeeAlsoNodes.Count > 0)) Then
            MdxCnt = SeeAlsoNodes.Count * 3
            For m As Int16 = 0 To SeeAlsoNodes.Count - 1
                If ((String.Compare(SeeAlsoNodes(m).InnerText, "see", True) = 0) Or (String.Compare(SeeAlsoNodes(m).InnerText, "see also", True) = 0)) Then
                    If ((SeeAlsoNodes(m).ParentNode IsNot Nothing) AndAlso (SeeAlsoNodes(m).NextSibling IsNot Nothing)) Then
                        If ((SeeAlsoNodes(m).ParentNode.InnerXml.Contains("<see-entry ")) Or (SeeAlsoNodes(m).ParentNode.InnerXml.Contains("<see-also-entry "))) Then
                            SeeAlsoNodes(m).ParentNode.RemoveChild(SeeAlsoNodes(m))
                        End If
                    End If
                End If
            Next
            ...
            For b As Int16 = 0 To BookPartNodes.Count - 1
                ...
                Try
                    ...
                    If (Not AbstNode.OuterXml.Contains("<abstract")) Then
                        BookPartNodes(b).InnerXml = $"{BookPartNodes(b).InnerXml}<abstract abstract-type=""abstract"">{AbstNode.OuterXml}</abstract>"
                    Else
    ...
    Private Function PublisherElement(ByVal MetadatNode As XmlNode) As Boolean
        ...
        Dim BookMetaNode As XmlNode = xmlMeta.SelectSingleNode("//book-meta")
        Dim PublisherNameNodes As XmlNodeList = BookMetaNode.SelectNodes("//publisher/phrase") '30-12-2021
        'Dim PublisherNameNodes As XmlNodeList = BookMetaNode.SelectNodes("//copyright/holder")
        ...
        Try
            xmlMeta.MoveXMLNode("//copyright[child::holder]", "//imprint-meta[child::imprint-text]", NodeMoveOption.FIRSTCHILD)
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        End Try

        If ((PublisherNameNodes Is Nothing) OrElse (PublisherNameNodes.Count = 0)) Then
            GBL.DeantaBallon("No copyright/holder information found. So the tool is not split the publisher-loc element.", MessageType.MSGERROR)
            Return False
        End If
        If (PublisherNameNodes.Count >= 1) Then
            PublisherName = PublisherNameNodes(0).InnerText.Replace("by ", "")
            PublisherName = PublisherNameNodes(1).InnerText.Replace("and by ", "")
        ...
        For p As Integer = 0 To PublisherLocNodes.Count - 1
            If (IsSinglePublisher) Then
                PublisherXML = PublisherXML & $"<publisher><publisher-name>{PublisherName.Replace("&", "&amp;")}</publisher-name><publisher-loc>{PublisherLocNodes(p).InnerText}</publisher-loc></publisher>"
            Else
                PublisherXML = PublisherXML & $"<publisher><publisher-name>{PublisherName.Replace("&", "&amp;")}</publisher-name><publisher-loc>{PublisherLocNodes(p).InnerText}</publisher-loc></publisher>"
            End If
        Next
    ...
    Private Function AddAuthorInformation() As Boolean
        ...
        If (ContribNode Is Nothing) Then
            GBL.DeantaBallon($"No book author found. So affiliation and bio author not updated.", MessageType.MSGERROR)
            Return False
        End If
        ...
        Dim BioNodeList As XmlNodeList = xmlMeta.SelectNodes("//bio[count(child::para)=0]")
        If ((BioNodeList IsNot Nothing) AndAlso (BioNodeList.Count > 0)) Then
            For b As Int16 = 0 To BioNodeList.Count - 1
                If ((Not BioNodeList(b).InnerXml.Trim().StartsWith("<p")) Or (Not BioNodeList(b).InnerXml.Trim().EndsWith("</p>"))) Then
                    BioNodeList(b).InnerXml = $"<p>{BioNodeList(b).InnerXml}</p>"
                End If
            Next
        End If
        ...
        If ((ImprintMetaNode IsNot Nothing) AndAlso (AbstrctNode IsNot Nothing)) Then
            ...
            End If
        End If
        AbstrctNode = xmlMeta.SelectSingleNode("//imprint-meta")
    ...
    Private Function GruopBodyforChapter(ByVal xmlString As String) As String
        ...
        Dim FirstSectionList As XmlNodeList = xmlDoc.SelectNodes("//chapter/section[1]|//chapter/para[1]|//chapter/sidebar[1]|//chapter/epigraph", NameSpaceManager)
    ...
    Private Function IndexXmlCleanup(ByVal XmlString As String) As String
        ...
        '15-12-2021
        Dim SemiRegx As Regex = Nothing
            Dim Idx As Int16 = 0
            Dim IndxMatch As Match = Nothing
            Dim SearchIndex As Integer = 0
            Dim IndexPos As Integer = 0
            Dim OrgIndexXML As String = String.Empty
            Dim ProcessXML As String = String.Empty
        ...
        If (Not IsePubXML) Then
            If ((PrimaryLst IsNot Nothing) AndAlso (PrimaryLst.Count > 0)) Then
                For p As Int16 = 0 To PrimaryLst.Count - 1
                    OrgIndexXML = String.Empty
                    Idx = 0
                    IndexPos = 0
                    Try
                        If (PrimaryLst(p).InnerXml.Contains(">;")) Then
                            OrgIndexXML = PrimaryLst(p).InnerXml
                            Dim SeeInx As Int16 = GetSeeTermIndex(OrgIndexXML)
                            SemiRegx = New Regex(">[\s]*;", RegexOptions.Singleline Or RegexOptions.IgnoreCase)
                            IndxMatch = SemiRegx.Match(OrgIndexXML, SearchIndex)
                            While (IndxMatch.Success)
                                If ((SeeInx <> -1) And (SeeInx = Idx)) Then
                                    Exit For
                                End If
                                If (Idx = 0) Then
                                    OrgIndexXML = OrgIndexXML.Insert(IndxMatch.Captures(0).Index + IndxMatch.Value.Length, "<indexterm><secondary><term>")
                                Else
                                    OrgIndexXML = OrgIndexXML.Insert(IndxMatch.Captures(0).Index + IndxMatch.Value.Length, "</term></secondary></indexterm><indexterm><secondary><term>")
                                End If
                                IndexPos = IndxMatch.Captures(0).Index + IndxMatch.Value.Length + 14
                                IndxMatch = SemiRegx.Match(OrgIndexXML, IndexPos)
                                Idx = Idx + 1
                            End While
                            If ((PrimaryLst(p).InnerXml.StartsWith("<term>")) And (PrimaryLst(p).InnerXml.EndsWith("</term>"))) Then
                                OrgIndexXML = $"{OrgIndexXML}</secondary></indexterm></term>"
                                'OrgIndexXML = OrgIndexXML.Replace("</link>;<indexterm>", ";</link><indexterm>")
                            End If
                            If (PrimaryLst(p).ParentNode IsNot Nothing) Then
                                PrimaryLst(p).ParentNode.InnerXml = PrimaryLst(p).ParentNode.InnerXml.Replace(PrimaryLst(p).OuterXml, PrimaryLst(p).OuterXml.Replace(PrimaryLst(p).InnerXml, OrgIndexXML))
                            End If
                        End If
                    Catch ex As Exception
                        GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        ...
        PrimaryLst = xmlDoc.SelectNodes("//primary//emphasis[@role='italic']")
        If ((PrimaryLst IsNot Nothing) AndAlso (PrimaryLst.Count > 0)) Then
            For p As Int16 = 0 To PrimaryLst.Count - 1
                Try
                    If (Regex.IsMatch(PrimaryLst(p).InnerText, "see also (\w)+", RegexOptions.Singleline Or RegexOptions.IgnoreCase)) Then
                        InnerXML = $"<emphasis role=""italic"">{PrimaryLst(p).InnerXml.Replace("see also", "see also</emphasis>")}"
                        If (PrimaryLst(p).ParentNode IsNot Nothing) Then
                            PrimaryLst(p).ParentNode.InnerXml = PrimaryLst(p).ParentNode.InnerXml.Replace(PrimaryLst(p).OuterXml, InnerXML)
                        End If
                    End If
                Catch ex As Exception
                    GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        ...
        '30-06-2021
        Dim FigureNodes As XmlNodeList = xmlDoc.SelectNodes("//figure")
        If ((FigureNodes IsNot Nothing) AndAlso (FigureNodes.Count > 0)) Then
            For f As Int16 = 0 To FigureNodes.Count - 1
                Try
                    Dim MediaNodes As XmlNodeList = FigureNodes(f).SelectNodes(".//mediaobject")
                    If ((MediaNodes IsNot Nothing) AndAlso (MediaNodes.Count > 0)) Then
                        If (MediaNodes.Count = 1) Then
                            Try
                                Dim XMLId As XmlAttribute = xmlDoc.CreateNode(XmlNodeType.Attribute, "id", "")
                                XMLId.Value = FigureNodes(f).Attributes("xml:id").Value
                                MediaNodes(0).Attributes.Append(XMLId)
                            Catch ex As Exception
                            End Try
                        ElseIf (MediaNodes.Count > 1) Then
                            For m As Int16 = 0 To MediaNodes.Count - 1
                                Dim XMLId As XmlAttribute = xmlDoc.CreateNode(XmlNodeType.Attribute, "id", "")
                                Dim Imagedata As XmlNode = MediaNodes(m).SelectSingleNode(".//imagedata")
                                If (Imagedata IsNot Nothing) Then
                                    Try
                                        XMLId.Value = Path.GetFileNameWithoutExtension(Imagedata.Attributes("fileref").Value).Replace("fig", "F")
                                    Catch ex As Exception
                                    End Try
                                End If
                                MediaNodes(m).Attributes.Append(XMLId)
                            Next
                        End If
                    End If
                Catch ex As Exception
        ...
        Dim IndexPageNodes As XmlNodeList = Nothing
        Dim LastPageNumber As String = String.Empty

        If (IsePubXML) Then '23-09-2021
            IndexPageNodes = xmlDoc.SelectNodes("//a[@id]")
        Else
            IndexPageNodes = xmlDoc.SelectNodes("//index//a[@id]")
        End If
        ...
        If ((IndexNode IsNot Nothing) AndAlso (IndexNode.Count > 0)) Then
            If (Not IsePubXML) Then '23-09-2021
        ...
        '23-04-2021
        PrimaryLst = xmlDoc.SelectNodes("//primary|//secondary|//tertiary")
            InnerXML = String.Empty
            If ((PrimaryLst IsNot Nothing) AndAlso (PrimaryLst.Count > 0)) Then
                For pg As Integer = 0 To PrimaryLst.Count - 1
                    If (Not PrimaryLst(pg).InnerXml.Contains("<link")) Then Continue For
                Dim Indexs As XmlNodeList = PrimaryLst(pg).SelectNodes(".//indexterm")

                If ((Indexs Is Nothing) OrElse (Indexs.Count = 0)) Then
                        InnerXML = PrimaryLst(pg).InnerXml
                        Try
                        ...
                        Catch ex As Exception
                            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
                            PrimaryLst(pg).InnerXml = InnerXML
                        End Try
                    Else
                        For p As Int16 = 0 To Indexs.Count - 1
                            If (Not Indexs(p).InnerXml.Contains("<link")) Then Continue For
                            InnerXML = Indexs(p).InnerXml
                            Try
                                Indexs(p).InnerXml = Regex.Replace(Indexs(p).InnerXml.Replace("</term>", ""), "(<term>(((?!<link).)*))", "$1</term>", RegexOptions.Singleline Or RegexOptions.IgnoreCase).Replace("</link>; <link role=""page"">", "</link>, <link role=""page"">")
                            Catch ex As Exception
                                GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
                                Indexs(p).InnerXml = InnerXML
                            End Try
                        Next
                        InnerXML = PrimaryLst(pg).InnerXml
                        For Each mat As Match In Regex.Matches(PrimaryLst(pg).InnerXml, "(<term>(((?!<link).)*))")
                            Try
                                If (mat.Value.Contains("</term>")) Then Continue For
                                If (PrimaryLst(pg).InnerXml.StartsWith("<term>")) And (PrimaryLst(pg).InnerXml.EndsWith("</term>")) Then
                                    PrimaryLst(pg).InnerXml = PrimaryLst(pg).InnerXml.ReplaceLastOccurrence("</term>", "").Replace(mat.Value, $"{mat.Value}</term>")
                                End If
                            Catch ex As Exception
                                GBL.DeantaBallon($"Split term in indexterm-{ex.Message}", MessageType.MSGERROR)
                                Continue For
                            End Try
                        Next
                        'Try
        ...
        Try
            IndexSeeandSeealsoSplit(xmlDoc, "see")
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        End Try

        Try
            IndexSeeandSeealsoSplit(xmlDoc, "see also")
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        End Try
        ...
        If (Not IsePubXML) Then
            ...
            For pg As Integer = 0 To PrimaryList.Count - 1
                ...
                IsValid = False
                PrimaryList(pg).InnerXml = Regex.Replace(PrimaryList(pg).InnerXml.Replace("<term></term>", ""), "<term[\s]*/>", "", RegexOptions.Singleline Or RegexOptions.IgnoreCase)
                Try
                    If (Regex.Match(PrimaryList(pg).InnerXml, "(</link>)(;(?:(?!<link[^>]*>).)*)", RegexOptions.Singleline Or RegexOptions.IgnoreCase).Success) Then
                        ...
                        Dim LinkRegMatches As MatchCollection = Regex.Matches(PrimaryList(pg).InnerXml, "(</link>)([;\s]+)((?:(?!<link[^>]*>).)*)", RegexOptions.Singleline Or RegexOptions.IgnoreCase)
                        For lk As Integer = 0 To LinkRegMatches.Count - 1
                            'If (LinkRegMatches(lk).Groups(2).Value.Contains("</term>")) Then
                            If (From n In LinkRegMatches(lk).Groups Where n.value.Contains("</term>") Select n).Any Then
                                Continue For
                            End If
                            MatchXML = LinkRegMatches(lk).Value
                            '24-01-2022
                            ' skip term for see and see aslo text
                            'If (Regex.Match(MatchXML, "<emphasis[^>]*>see", RegexOptions.Singleline Or RegexOptions.IgnoreCase).Success) Then
                            '    Exit For
                            'End If
                            '24-01-2022
                            If ((Not String.IsNullOrEmpty(LinkRegMatches(lk).Groups(2).Value.TrimStart(" ;"))) Or (Not String.IsNullOrEmpty(LinkRegMatches(lk).Groups(2).Value.TrimStart("; ")))) Then
                                If (lk = 0) Then
                                    TermXml = $"{LinkRegMatches(lk).Groups(1).Value}{LinkRegMatches(lk).Groups(2).Value}<indexterm><secondary><term>{ LinkRegMatches(lk).Groups(3).Value}</term>"
                                    'TermXml = String.Format("{0}<indexterm><secondary><term>{1}</term>", LinkRegMatches(lk).Groups(1).Value, LinkRegMatches(lk).Groups(2).Value.TrimStart(";").TrimStart(" ")) '30-04-2021
                                Else
                                    'TermXml = String.Format("{0}</secondary></indexterm><indexterm><secondary><term>{1}</term>", LinkRegMatches(lk).Groups(1).Value, LinkRegMatches(lk).Groups(2).Value.TrimStart(";").TrimStart(" "))  '30-04-2021
                                    TermXml = $"{LinkRegMatches(lk).Groups(1).Value}</secondary></indexterm>{LinkRegMatches(lk).Groups(2).Value}<indexterm><secondary><term>{LinkRegMatches(lk).Groups(3).Value}</term>"
                                End If
                        ...
                        If (IsValid) Then
                            Try
                                ...
                                PrimaryXML = Regex.Replace(PrimaryXML, "<term[\s]*/>", "", RegexOptions.Singleline Or RegexOptions.IgnoreCase)
                                PrimaryXML = PrimaryXML.Replace("<term></term>", "")
                                If (Not Regex.IsMatch(PrimaryXML, "<term>[\s]*<emphasis[^>]*>see", RegexOptions.Singleline Or RegexOptions.IgnoreCase)) Then
                                    PrimaryList(pg).InnerXml = PrimaryList(pg).InnerXml.Replace(OrgPrimaryXML, PrimaryXML)
                                End If
                            Catch ex As Exception
                                PrimaryList(pg).InnerXml = OrgPrimaryXML
                                GBL.DeantaBallon($"split secondary term. - {ex.Message}", MessageType.MSGERROR)
                            End Try
        ...
        Dim IndextermEntry As XmlNodeList = xmlDoc.SelectNodes("//indexterm")
        If ((IndextermEntry IsNot Nothing) AndAlso (IndextermEntry.Count > 0)) Then
            For i As Int16 = 0 To IndextermEntry.Count - 1
                Try
                    IndextermEntry(i).Attributes("xml:id").Value = $"see{i + 1}"
                Catch ex As Exception
                    If ((IndextermEntry(i).Attributes Is Nothing) OrElse (IndextermEntry(i).Attributes.Count = 0)) Then
                        Dim indexAtt As XmlAttribute = xmlDoc.CreateNode(XmlNodeType.Attribute, "xml:id", "xml:")
                        indexAtt.Value = $"see{i + 1}"
                        IndextermEntry(i).Attributes.Prepend(indexAtt)
                    End If
                End Try
            Next
        End If
        ...
        Try
            IndexSeeandSeealsoLinking(xmlDoc, "see also", "see-also-entry")
            IndexSeeandSeealsoLinking(xmlDoc, "see", "see-entry")
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        End Try
        ...
        If ((OutsidePageNodes IsNot Nothing) AndAlso (OutsidePageNodes.Count > 0)) Then
            For o As Int32 = 0 To OutsidePageNodes.Count - 1
                Try
                    If ((OutsidePageNodes(o).NextSibling IsNot Nothing) AndAlso (OutsidePageNodes(o).NextSibling.NodeType = XmlNodeType.Element)) Then
        ...
        If ((ListItems IsNot Nothing) AndAlso (ListItems.Count > 0)) Then
            For lt As Integer = 0 To ListItems.Count - 1
                If ((ListItems(lt).PreviousSibling IsNot Nothing) AndAlso (ListItems(lt).PreviousSibling.NodeType = XmlNodeType.Element)) Then
        ...
        Dim PrevNode As XmlNode = Nothing
        Dim PrevCnt As Int32 = 0
        If ((PageTagList IsNot Nothing) AndAlso (PageTagList.Count > 0)) Then
            For pg As Integer = 0 To PageTagList.Count - 1
                'If (PageTagList(pg).OuterXml.Contains("_170")) Then
                '    MsgBox("OK")
                'End If
                PrevCnt = 0
                Try
                    PrevNode = PageTagList(pg).PreviousSibling
                    While ((PrevNode IsNot Nothing) AndAlso (PrevNode.NodeType <> XmlNodeType.Element))
                        If (PrevCnt = 4) Then Exit While
                        PrevNode = PrevNode.PreviousSibling
                        PrevCnt = PrevCnt + 1
                    End While
                    If ((PrevNode IsNot Nothing) AndAlso (PrevNode.NodeType <> XmlNodeType.Element)) Then
                        Continue For
                    End If
                Catch ex As Exception
                    GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
                ...
                PrevCnt = 0
                While (PrevCnt <= 4)
                    If ((PrevNode IsNot Nothing) AndAlso ((String.Compare(PrevNode.Name, "para", True) = 0) Or (String.Compare(PrevNode.Name, "bibliomixed", True) = 0) Or (String.Compare(PrevNode.Name, "source", True) = 0))) Then
        ...
        If ((PageTagList IsNot Nothing) AndAlso (PageTagList.Count > 0)) Then
            For pg As Integer = 0 To PageTagList.Count - 1
                ...
                If (PageTagList(pg).NextSibling Is Nothing) OrElse (PageTagList(pg).NextSibling.NodeType <> XmlNodeType.Element) Then
                    Continue For
                End If
                PrevNode = PageTagList(pg).NextSibling
                While (PrevCnt <= 4)
    ...
    Private Function GetSeeTermIndex(ByVal Termtext As String) As Int16
        Dim Idx As Int16 = -1
        Dim SplitTerms As New List(Of String)
        SplitTerms.AddRange(Regex.Split(Termtext, ">[\s]*;"))
        If ((SplitTerms Is Nothing) OrElse (SplitTerms.Count = 0)) Then
            Return Idx
        End If

        Idx = SplitTerms.FindIndex(Function(spt)
                                       If (spt.Contains("<emphasis role=""italic"">see also</emphasis>")) Then
                                           Return True
                                       End If
                                       Return False
                                   End Function)
        Return Idx
    End Function
    Private Function OnpageFootnoteReorderFootnote(ByVal Content As String, ByVal FileName As String) As String
        Dim xmlInddDoc As New XmlDocument
        xmlInddDoc.PreserveWhitespace = True
        Dim Label As String = String.Empty
        Try
            xmlInddDoc.LoadXml(Content.Replace("&", "&amp;").Replace("xmlns=""http://docbook.org/ns/docbook""", ""))
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
            Return String.Empty
        End Try
        Dim FootnoteList As XmlNodeList = xmlInddDoc.SelectNodes("//footnote[@role='end-ch-note']")
        If ((FootnoteList Is Nothing) OrElse (FootnoteList.Count = 0)) Then
            Return Content
        End If
        GBL.DeantaBallon($"Footnote Re-number started. {FileName}", MessageType.MSGINFO)
        For f As Integer = 0 To FootnoteList.Count - 1
            ...
            If (Not (String.Compare(Label, $"{f + 1}", True) = 0)) Then
                GBL.DeantaBallon($"Footnote label attribute and display text mismatched. {FootnoteList(f).OuterXml}", MessageType.MSGERROR)
                FootnoteList(f).Attributes("label").Value = $"{f + 1}"
            End If
        Next
        Return xmlInddDoc.OuterXml.Replace("&amp;", "&")
    End Function
            ...
            While (((FootnoteList IsNot Nothing) AndAlso (FootnoteList.Count > 0)))
                ...
                Try
                    ...
                    Try
                        ...
                        FtNode.InnerXml = Regex.Replace(SecFtNote.InnerXml, $"^({ParaXml})({Label}|([0-9]+))[.\\t]*", "$1", RegexOptions.IgnoreCase Or RegexOptions.Singleline)
                    Catch ex As Exception
                        GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
                        FootnoteIndex = FootnoteIndex + 1
                        Continue While
                    ...
                    If (FtNode.Attributes("xml:id") Is Nothing) Then
                        ...
                        End If
                    End If
                    If ((SecFtNote IsNot Nothing) AndAlso (SecFtNote.ParentNode IsNot Nothing)) Then
                        SecFtNote.ParentNode.RemoveChild(SecFtNote)
                    End If
                Catch ex As Exception
                    GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
    ...
    Private Function IndexSeeandSeealsoSplit(ByVal xmlIndex As XmlDocument, ByVal IndexSeeText As String) As Boolean
        Dim PrimaryList As XmlNodeList = xmlIndex.SelectNodes("//term")
        ...
        For p As Int32 = 0 To PrimaryList.Count - 1
            ...
            Try
                If (Not PrimaryList(p).InnerXml.Contains($" <emphasis role=""italic"">{IndexSeeText}</emphasis>")) Then Continue For
                If (PrimaryList(p).SelectNodes(".//term").Count > 0) Then Continue For
                If (PrimaryList(p).ParentNode IsNot Nothing) Then
                    Dim SplitXML As String = PrimaryList(p).OuterXml.Replace($"</{PrimaryList(p).Name}>", "").Replace($" <emphasis role=""italic"">{IndexSeeText}</emphasis>", $"</{PrimaryList(p).Name}> <emphasis role=""italic"">{IndexSeeText}</emphasis>")
                    If (Not String.IsNullOrEmpty(SplitXML)) Then
                        PrimaryList(p).ParentNode.InnerXml = PrimaryList(p).ParentNode.InnerXml.Replace(PrimaryList(p).OuterXml, SplitXML)
                    End If
                End If
            Catch ex As Exception
                GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
    ...
    Private Function IndexSeeandSeealsoLinking(ByVal xmlIndex As XmlDocument, ByVal IndexSeeText As String, ByVal TagName As String) As Boolean
        Dim PrimaryList As XmlNodeList = xmlIndex.SelectNodes("//primary|//secondary")
        If ((PrimaryList Is Nothing) OrElse (PrimaryList.Count = 0)) Then Return False
        Dim IndexTerms As New List(Of IndexSeeData)
        Dim DestIndexID As String = String.Empty
        Dim IndexSeeIndex As Int32 = 0
        For p As Int32 = 0 To PrimaryList.Count - 1
            If (IndexSeeText = "see") Then
                If ((PrimaryList(p).InnerText.Contains(IndexSeeText)) And (PrimaryList(p).InnerText.Contains("see also"))) Then
                    Continue For
                End If
            End If
            ...
            DestIndexID = Nothing
#If CONFIG = "FinalXML" Then
            'If (PrimaryList(p).InnerText.Contains("gender awareness initiatives")) Then
            '    MsgBox("ok")
            'End If
#End If
            Try
            ...
            For t As Int16 = IndexTerms.Count - 1 To 0 Step -1
                ...
                Try
                    ...
                    '23-12-2021
                    If (Regex.Match(PrimaryXML, Regex.Escape(IndexTerm), RegexOptions.Singleline Or RegexOptions.IgnoreCase).Success) Then
                        PrimaryXML = SearchRegex.Replace(PrimaryXML, $"<{TagName} rid=""{DestIndexID}"">{IndexTerm}</{TagName}>", 1, IndexSeeIndex)
                    Else
                        SearchRegex = New Regex(GeneratePattern(IndexTerm), RegexOptions.IgnoreCase Or RegexOptions.Singleline)
                        If (Regex.IsMatch(PrimaryXML, GeneratePattern(IndexTerm), RegexOptions.Singleline Or RegexOptions.IgnoreCase)) Then
                            PrimaryXML = SearchRegex.Replace(PrimaryXML, $"<{TagName} rid=""{DestIndexID}"">{IndexTerm}</{TagName}>", 1, IndexSeeIndex)
                        Else
                            GBL.DeantaBallon($"See term is not found. - {IndexTerm}", MessageType.MSGERROR)
                            Continue For
                        End If
                    End If
                    '23-12-2021
    ...
    Private Function EscapeRegCharacter(InputChar As String) As String
        Dim EscapeCharas As New List(Of String)(New String() {"(", ")", "[", "]", "{", "}", "/", "-", "?", "$", "#", ".", "+", "*", "^"})
        InputChar = InputChar.Replace("?", "::").Replace("\", "\\")
        EscapeCharas.ForEach(Sub(ch As String)
                                 InputChar = InputChar.Replace(ch, "\" & ch)
                             End Sub)
        InputChar = InputChar.Replace("::", ".")
        Return InputChar
    End Function
    Private Function GeneratePattern(ByVal firstChar As String) As String
        Dim Pattern As String = ""
        For Each chr As String In firstChar.Trim().Split(" ")
            Pattern = IIf(String.IsNullOrEmpty(Pattern), EscapeRegCharacter(chr), Pattern & "(?:(?:(?:\s)*<(?:[^>]+)>(?:\s)*)*) " & EscapeRegCharacter(chr))
        Next
    ...
    Private Function FindDestinationIndexTerm(ByVal xmlIndex As XmlDocument, ByVal IndexTerm As String) As String
        ...
        TmpIndxTerm = TmpIndxTerm.Replace("&amp;", "&")
            ...
            For s As Int16 = 0 To SeeTextList.Count - 1
                Try
                        ...
                        If ((SeeTextSplit IsNot Nothing) AndAlso (SeeTextSplit.Count > 0)) Then
                            For t As Int16 = 0 To SeeTextSplit.Count - 1
                                Try
                                    ...
                                    Dim tmpSearch As String = TmpSee.Replace("[", "\[").Replace("]", "\]").Replace("(", "\(").Replace(")", "\)")
                                    If (SeeTextSplit(t).Contains(IndexSeeText)) Then
                                        SeeXMLIndex = Regex.Match(NodeXML, tmpSearch.Replace(IndexSeeText, $"<emphasis role=""italic"">( )?{IndexSeeText}</emphasis>"), RegexOptions.Singleline Or RegexOptions.IgnoreCase).Index
                                    ...
                                    Else
                                        ...
                                        If (SeeXMLIndex = -1) Then
                                            SeeXMLIndex = Regex.Match(NodeXML, GeneratePattern(TmpSee), RegexOptions.Singleline Or RegexOptions.IgnoreCase).Index
                                        End If

                                    End If
                                    If (SeeXMLIndex > 0) Then
                                        IndexTermList.Add(New IndexSeeData With {.SeeTerm = TmpSee.Replace($"{IndexSeeText} ", "").Trim(), .SeeIndex = SeeXMLIndex})
                                    End If
                                Catch ex As Exception
                                    GBL.DeantaBallon($"New_SplitIndexTerms1 - {ex.Message}", MessageType.MSGERROR)
                                    Continue For
                                End Try
                            Next
                        End If
                    Else
                        ...
                        Dim tmpSearch As String = SeeText.Replace("[", "\[").Replace("]", "\]").Replace("(", "\(").Replace(")", "\)")
        ...
        For bb As Integer = 0 To BiblioList.Count - 1
            Try
                ...
                If ((BiblioList(bb).InnerXml.Contains("<publisher-loc>") Or BiblioList(bb).InnerXml.Contains("<publisher-name>") Or BiblioList(bb).InnerXml.Contains("<chapter-title>"))) Then
                    ...
                    'If ((BiblioList(bb).InnerXml.Contains("<volume>") Or BiblioList(bb).InnerXml.Contains("<issue>"))) Then  '30-12-2021
                    '    Continue For
                    'End If

                    If (BiblioList(bb).InnerXml.Contains("<article-title>")) Then
                        BiblioList(bb).InnerXml = BiblioList(bb).InnerXml.Replace("<article-title>", "<chapter-title>").Replace("</article-title>", "</chapter-title>")
                    End If
    ...
    Private Function TandFXmlCleanup(ByVal XmlString As String) As String
        ...
        Try
            ...
            XmlString = Regex.Replace(XmlString, "<dialogue[^>]*>(((?!<\/dialogue>).)*)</dialogue>", Function(mt As Match)
                                                                                                         ...
                                                                                                         TmpDialogue = TmpDialogue.Replace("</speaker> <line", "</speaker><line").Replace("</speaker>&#x2003;<line", "&#x2003;</speaker><line")
                                                                                                         TmpDialogue = TmpDialogue.Replace("<line>", "<speech><line>").Replace("</line>", "</line></speech>").Replace("<line ", "<speech><line ")
                                                                                                         TmpDialogue = TmpDialogue.Replace("<speaker>", "<speech><speaker>").Replace("</speaker>", "</speaker></speech>")
                                                                                                         TmpDialogue = TmpDialogue.Replace("</speech><speech><line>", "<line>").Replace("</speech><speech><line ", "<line ").Replace("</speaker></speech><speech>", "</speaker>")
                                                                                                         Return TmpDialogue
                                                                                                     End Function, RegexOptions.Singleline Or RegexOptions.IgnoreCase)
        Catch ex As Exception
        ...
        Dim EmptyRows As XmlNodeList = xmlDoc.SelectNodes("//row[count(child::entry)=1][not (./descendant::informalfigure)]")
        If ((EmptyRows IsNot Nothing) AndAlso (EmptyRows.Count > 0)) Then
            For e As Int16 = 0 To EmptyRows.Count - 1
                If (String.IsNullOrEmpty(EmptyRows(e).InnerText)) Then
                    If (EmptyRows(e).ParentNode IsNot Nothing) Then
                        EmptyRows(e).ParentNode.RemoveChild(EmptyRows(e))
                    End If
                End If
            Next
        End If
        ...
        If (Not IsePubXML) Then
            Try
                RetainAsChapter(xmlDoc, "//chapter/glossary", "glossary")
            Catch ex As Exception
                GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
            End Try

            Try
                RetainAsChapter(xmlDoc, "//chapter/bibliography", "bibliography")
            Catch ex As Exception
                GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
            End Try

            Dim TableCase As XmlNode = xmlDoc.SelectSingleNode("//chapter/title")
            If (TableCase IsNot Nothing) Then
                If (((TableCase.InnerText = "Table of Cases") Or (TableCase.InnerText = "Table of Legislation")) AndAlso (TableCase.ParentNode IsNot Nothing) And (TableCase.ParentNode.ParentNode IsNot Nothing)) Then
                    Dim TocContent As String = Regex.Replace(TableCase.ParentNode.InnerXml, "<section[^>]*>", "<toc-div><toc-entry><p>", RegexOptions.Singleline Or RegexOptions.IgnoreCase).Replace("</section>", "</p></toc-entry></toc-div>")
                    TocContent = TocContent.Replace("<title", "<toc-title-group><title").Replace("</title>", "</title></toc-title-group>")
                    TableCase.ParentNode.ParentNode.InnerXml = TableCase.ParentNode.ParentNode.InnerXml.Replace(TableCase.ParentNode.OuterXml, $"<toc content-type=""law-table"">{TocContent}</toc>")
                    xmlDoc.RemoveXMLNodes("//toc/alt-title")
                End If
            End If
        End If

        Dim RemarkNodes As XmlNodeList = xmlDoc.SelectNodes("//remark")
        If ((RemarkNodes IsNot Nothing) AndAlso (RemarkNodes.Count > 0)) Then
            For r As Int16 = 0 To RemarkNodes.Count - 1
                Dim ChtitleNode As XmlNode = RemarkNodes(r).SelectSingleNode("./ancestor::part/label|./ancestor::chapter/title")
                If ((ChtitleNode IsNot Nothing) AndAlso (ChtitleNode.ParentNode IsNot Nothing)) Then
                    ChtitleNode.ParentNode.InsertBefore(RemarkNodes(r), ChtitleNode)
                End If
            Next
        End If

        Dim TblFnNodes As XmlNodeList = xmlDoc.SelectNodes("//table[child::tblfn][not (child::tblfoot)]")
        If ((TblFnNodes IsNot Nothing) AndAlso (TblFnNodes.Count > 0)) Then
            For t As Int16 = 0 To TblFnNodes.Count - 1
                Try
                    TblFnNodes(t).InnerXml = TblFnNodes(t).InnerXml.Replace("<tblfn", "<tblfoot><tblfn").Replace("</tblfn>", "</tblfn></tblfoot>").Replace("</tblfoot><tblfoot>", "")
                Catch ex As Exception
                End Try
            Next
        End If

        TblFnNodes = xmlDoc.SelectNodes("//tblfn")
        If ((TblFnNodes IsNot Nothing) AndAlso (TblFnNodes.Count > 0)) Then
            For t As Int16 = 0 To TblFnNodes.Count - 1
                TblFnNodes(t).InnerXml = TblFnNodes(t).InnerXml.Replace(vbTab, " ")
            Next
        End If


        Dim NameStNodes As XmlNodeList = xmlDoc.SelectNodes("//entry[@namest][@nameend]")
        Dim NameSt As Int16 = 0
        Dim NameEnd As Int16 = 0
        If ((NameStNodes IsNot Nothing) AndAlso (NameStNodes.Count > 0)) Then
            For n As Int16 = 0 To NameStNodes.Count - 1
                NameEnd = 0
                NameSt = 0
                Try
                    NameSt = NameStNodes(n).Attributes("namest").Value
                Catch ex As Exception
                    NameSt = 0
                End Try
                Try
                    NameEnd = NameStNodes(n).Attributes("nameend").Value
                Catch ex As Exception
                    NameEnd = 0
                End Try
                If (NameSt = 0 Or NameEnd = 0) Then
                    Continue For
                End If
                Try
                    CType(NameStNodes(n), XmlElement).SetAttribute("colspan", (NameEnd - NameSt) + 1)
                Catch ex As Exception
                End Try
            Next
        End If

        Dim ColspanList As XmlNodeList = xmlDoc.SelectNodes("//entry[@colspan]")
        Dim ColSpan As String = String.Empty
        Dim PrecedingCnt As Integer = 0
        If ((ColspanList IsNot Nothing) AndAlso (ColspanList.Count > 0)) Then
            For Each ColNode As XmlNode In ColspanList
                Dim TdNode As XmlElement = ColNode
                'If (TdNode.InnerText.Contains("Elevation")) Then
                '    MsgBox("OK")
                'End If
                PrecedingCnt = 0
                ColSpan = 0
                ColSpan = ColNode.Attributes("colspan").Value
                PrecedingCnt = ColNode.SelectNodes(".//preceding-sibling::entry").Count
                Dim ColSpanNd As XmlNodeList = ColNode.SelectNodes(".//preceding-sibling::entry[@colspan]")
                If ((ColSpanNd IsNot Nothing) AndAlso (ColSpanNd.Count > 0)) Then
                    For Each nd As XmlNode In ColSpanNd
                        PrecedingCnt = PrecedingCnt + (nd.Attributes("colspan").Value - 1)
                    Next
                    PrecedingCnt = PrecedingCnt + 1
                Else
                    PrecedingCnt = PrecedingCnt + 1
                End If
                Try
                    TdNode.SetAttribute("namest", PrecedingCnt)
                    TdNode.SetAttribute("nameend", PrecedingCnt + (Convert.ToInt32(ColSpan) - 1))
                Catch ex As Exception
                    TdNode.Attributes("namest").Value = PrecedingCnt
                    TdNode.Attributes("nameend").Value = PrecedingCnt + (Convert.ToInt32(ColSpan) - 1)
                End Try
            Next
        End If
        ...
        If ((Urls IsNot Nothing) AndAlso (Urls.Count > 0)) Then
            For u As Int16 = 0 To Urls.Count - 1
                Try
                    If (Urls(u).InnerText.Contains("doi")) Then Continue For
                    If (Urls(u).InnerText.StartsWith("10.")) Then Continue For
        ...
        If ((PageTagList IsNot Nothing) AndAlso (PageTagList.Count > 0)) Then
            ChapterPartNode = xmlDoc.SelectSingleNode("//preface/para[1]|//primary[1]|//chapter/title|//part/title|//preface/title|//index/title|//bibliography/title|//abbreviation/title|//glossary/title|//toc-title-group/title")
        ...
        If (IsePubXML) Then
            ...
            'As per Ram advise to remove italic for Bloombury [MEMO]
        ...
        For u As Int16 = 0 To 3
            ...
            For t As Integer = 0 To txtList.Count - 1
                ...
                If ((String.Compare(txtList(t).ParentNode.Name, "uri", True) = 0) Or (String.Compare(txtList(t).ParentNode.Name, "link", True) = 0)) Then
                    Continue For
                End If
                If (Regex.IsMatch(txtList(t).InnerText.Replace("&amp;#x2005;", " "), "((http:|https:|www\.)([^ ><)]+))", RegexOptions.IgnoreCase Or RegexOptions.Singleline)) Then
                    Try
                        Dim tmpUrl As String = Regex.Replace(txtList(t).InnerText.Replace("&amp;#x2005;", " "), "((http:|https:|www\.)([^ ><)]+))", "<uri>$1</uri>", RegexOptions.IgnoreCase Or RegexOptions.Singleline)
                        txtList(t).ParentNode.InnerXml = txtList(t).ParentNode.InnerXml.Replace("&amp;#x2005;", " ").Replace(txtList(t).InnerText.Replace("&", "&amp;"), tmpUrl.Replace("&", "&amp;"))
                    Catch ex As Exception
                        Continue For
                    End Try
                End If
        ...
        For u As Int16 = 0 To 3
            Dim txtList As XmlNodeList = xmlDoc.SelectNodes("//bibliomixed/text()", NameSpaceManager)
            For t As Integer = 0 To txtList.Count - 1
                If (Regex.IsMatch(txtList(t).InnerText, "(doi:\s|\u00A0|&#x00A0;)?(10[.][0-9]{4,}(?:[.][0-9]+)*/(?:(?!["" &\'<>])\S)+)", RegexOptions.Singleline Or RegexOptions.IgnoreCase)) Then
                    Try
                        txtList(t).ParentNode.InnerXml = Regex.Replace(txtList(t).ParentNode.InnerXml, "(doi:\s|\u00A0|&#x00A0;)?(10[.][0-9]{4,}(?:[.][0-9]+)*/(?:(?!["" &\'<>])\S)+)", Function(MatDoi As Match)
                                                                                                                                                                                            Dim doi As String = MatDoi.Groups(2).Value.Trim()
                                                                                                                                                                                            If (doi.StartsWith("10.")) Then
                                                                                                                                                                                                Return $"{MatDoi.Groups(1).Value}<link xlink:href=""{doi}""><uri xlink:href=""{doi}"">{doi}</uri></link>"
                                                                                                                                                                                            Else
                                                                                                                                                                                                Return MatDoi.Value
                                                                                                                                                                                            End If
                                                                                                                                                                                        End Function, RegexOptions.Singleline Or RegexOptions.IgnoreCase)
        ...
        Dim InfoNode As XmlNode = xmlDoc.SelectSingleNode("//chapter/info")
        ...
        If (Not IsePubXML) Then
            ...
            Dim altText As String = String.Empty
                    ...
                    Try
                        ...
                        If ((informalfigureList(0).ChildNodes IsNot Nothing) AndAlso (informalfigureList(0).ChildNodes.Count > 0)) Then
                            ...
                            If (ImageData IsNot Nothing) Then
                                ...
                                End Try
                                Try
                                    altText = ImageData.Attributes("alt").Value
                                Catch ex As Exception
                                End Try
                        ...
                        If (Not String.IsNullOrEmpty(ImageName)) Then
                            If (InformalParent.ParentNode IsNot Nothing) Then
                                If (String.IsNullOrEmpty(altText)) Then
                                    InformalParent.ParentNode.InnerXml = InformalParent.ParentNode.InnerXml.Replace(InformalParent.OuterXml, "<inline-graphic xmlns:xlink=""http://www.w3.org/1999/xlink"" xlink:href=""" & ImageName & """ mime-subtype=""tif""/>")
                                Else
                                    InformalParent.ParentNode.InnerXml = InformalParent.ParentNode.InnerXml.Replace(InformalParent.OuterXml, "<inline-graphic xmlns:xlink=""http://www.w3.org/1999/xlink"" xlink:href=""" & ImageName & """ mime-subtype=""tif"" alt=""" & altText & """/>")
                                End If
                            End If
        ...
        If ((IsePubXML) And (OrgDocType <> DocumentType.CRITICALPUB) And (OrgDocType <> DocumentType.BLOOMSBURY) And (OrgDocType <> DocumentType.TRD)) Then
            LinkList = xmlDoc.SelectNodes("//link[@role='bibr']")
            'T&F 4.0 spec retainted the reference cross linking.
            'If (Not IsNumerRef) Then
            '    While ((LinkList IsNot Nothing) AndAlso (LinkList.Count > 0))
            '        Dim LinkNode As XmlNode = LinkList(0)
            '        Try
        ...
        UriList = xmlDoc.SelectNodes("//uri")
        If ((UriList IsNot Nothing) AndAlso (UriList.Count > 0)) Then
        ...
        XmlString = Regex.Replace(XmlString, "(<book)([^><]*)(>)", "$1$3", RegexOptions.IgnoreCase Or RegexOptions.Singleline)
        ...
        Return XmlString
    End Function
    Public Function RetainAsChapter(ByVal xmlDoc As XmlDocument, ByVal xPath As String, ByVal Tag As String) As Boolean
        Dim Glossary As XmlNode = xmlDoc.SelectSingleNode(xPath)
        If (Glossary IsNot Nothing) Then
            Dim GlosBook As XmlNode = Glossary.ParentNode.ParentNode
            Dim childNodes As List(Of String) = (From n As XmlNode In Glossary.ParentNode.ChildNodes Select n.Name).ToList()
            childNodes.Remove("alt-title")
            childNodes.Remove("alt-title")
            childNodes.Remove("title")
            If ((childNodes.Count = 1) AndAlso (childNodes(0) = Tag)) Then
                Dim Title As XmlNode = Glossary.ParentNode.SelectSingleNode(".//title")
                If (Title IsNot Nothing) Then
                    Glossary.PrependChild(Title)
                    If (GlosBook IsNot Nothing) Then
                        GlosBook.InnerXml = GlosBook.InnerXml.Replace(Glossary.ParentNode.OuterXml, Glossary.ParentNode.InnerXml)
                    End If
                    For g As Int16 = GlosBook.ChildNodes.Count To 0 Step -1
                        Try
                            If ((GlosBook.ChildNodes(g).Name <> Tag) And (GlosBook.ChildNodes(g).Name <> "a")) Then
                                GlosBook.ChildNodes(g).ParentNode.RemoveChild(GlosBook.ChildNodes(g))
                            End If
                        Catch ex As Exception
                        End Try
                    Next
        ...
        If ((CrossLinks IsNot Nothing) AndAlso (CrossLinks.Count > 0)) Then
            ...
            ElseIf (CrossLinks.Count > 2) Then
                For i As Integer = 0 To CrossLinks.Count - 1
                    If (i = 0) Then
                        If (IsSuperScriptFound) Then
                            CrossText = String.Format("<link role=""bibr"" linkend=""{0}_CIT{1}""><superscript>{2}</superscript></link><superscript>, </superscript>", IDAttrib, Convert.ToInt32(CrossLinks(i).Trim()).ToString("00000"), CrossLinks(i).Trim())
                        Else
                    ...
                    ElseIf ((i > 0) And (i <> CrossLinks.Count - 1)) Then
                        If (IsSuperScriptFound) Then
                            CrossText = CrossText & String.Format("<link role=""bibr"" linkend=""{0}_CIT{1}""><superscript>{2}</superscript></link><superscript>, </superscript>", IDAttrib, Convert.ToInt32(CrossLinks(i).Trim()).ToString("00000"), CrossLinks(i).Trim(), CrossLinks(i).Trim())
                        Else
                            CrossText = CrossText & String.Format("<link role=""bibr"" linkend=""{0}_CIT{1}"">{2}</link>, ", IDAttrib, Convert.ToInt32(CrossLinks(i).Trim()).ToString("00000"), CrossLinks(i).Trim(), CrossLinks(i).Trim())
                        End If

                    ElseIf (i = CrossLinks.Count - 1) Then
                        If (IsSuperScriptFound) Then
                            'CrossText = CrossText & "<superscript>&amp;#x2013;</superscript>" & String.Format("<link role=""bibr"" linkend=""{0}_CIT{1}""><superscript>{2}</superscript></link>", IDAttrib, Convert.ToInt32(CrossLinks(i).Trim()).ToString("00000"), CrossLinks(i).Trim()) '17-08-2022
                            CrossText = CrossText & String.Format("<link role=""bibr"" linkend=""{0}_CIT{1}""><superscript>{2}</superscript></link>", IDAttrib, Convert.ToInt32(CrossLinks(i).Trim()).ToString("00000"), CrossLinks(i).Trim()) '17-08-2022
                        Else
                            'CrossText = CrossText & "&amp;#x2013;" & String.Format("<link role=""bibr"" linkend=""{0}_CIT{1}"">{2}</link>", IDAttrib, Convert.ToInt32(CrossLinks(i).Trim()).ToString("00000"), CrossLinks(i).Trim()) '17-08-2022
                            CrossText = CrossText & String.Format("<link role=""bibr"" linkend=""{0}_CIT{1}"">{2}</link>", IDAttrib, Convert.ToInt32(CrossLinks(i).Trim()).ToString("00000"), CrossLinks(i).Trim()) '17-08-2022
                        End If
                    End If
                Next
    ...
    Private Function MathCleanUpforRL(ByVal EqnMat As Match) As String
        Dim Content As String = EqnMat.Value
        Content = Regex.Replace(Content, "<alternatives([^><]+)?>(((?!</alternatives>).)+)</alternatives>", "", RegexOptions.IgnoreCase Or RegexOptions.Singleline)
        Return Content
    End Function
    ...
    Private Sub FinalCleanup(Optional ByVal bXslExec As Boolean = False)
        Try
            ...
            XMLString = Regex.Replace(XMLString, "(<?xml version=""1.0""[^><]+>)", "$1" & Environment.NewLine & "<!--<!DOCTYPE book SYSTEM ""Z:\T_F-Books\general\TFB\TFB.dtd"">-->", RegexOptions.IgnoreCase Or RegexOptions.Singleline)
            ...
            'XMLString = Regex.Replace(XMLString, "<sec([^><]+)?><title([^><]+)?>Notes</title>((?:(?!</sec>).)+)(</sec>)</sec>", "<notes><title>Notes</title>$3$4</notes>", RegexOptions.IgnoreCase Or RegexOptions.Singleline) '28-07-2021

            XMLString = Regex.Replace(XMLString, "<sec([^><]+)?><title([^><]+)?>Notes</title>((?:(?!</sec>).)+)(</sec>)</sec>", Function(mt As Match)
                                                                                                                                    If (mt.Groups(3).Value.Contains("<footnote")) Then
                                                                                                                                        Return $"<notes><title>Notes</title>{mt.Groups(3).Value}{mt.Groups(4).Value}</notes>"
                                                                                                                                    Else
                                                                                                                                        Return mt.Value
                                                                                                                                    End If
            ...
            'XMLString = Regex.Replace(XMLString, "(doi:?)<uri(?:[^><]+)?>((?:(?!</uri>).)+)</uri>", "$1$2", RegexOptions.IgnoreCase Or RegexOptions.Singleline) ''03-02-2022
    ...
    Private Function MoveFigureandTable(ByVal xmlPath As String) As String
        ...
        'ElementList.Add(New MovePageNumData With {.ElementName = "boxed-text", .Position = PagePosition.SUFFIX}) '30-11-2021
        ...
        Dim GraphicNodes As XmlNodeList = Nothing
        ...
        If ((FigureLst IsNot Nothing) AndAlso (FigureLst.Count > 0)) Then
            For g As Integer = 0 To FigureLst.Count - 1
                Try
                    '30-06-2021
                    GraphicNodes = FigureLst(g).SelectNodes(".//graphic")
                    CaptionNode = FigureLst(g).SelectSingleNode(".//caption")
                    AltNode = FigureLst(g).SelectSingleNode("alt-text")
                    If ((GraphicNodes IsNot Nothing) AndAlso (GraphicNodes.Count > 0)) Then
                        For c As Int16 = GraphicNodes.Count - 1 To 0 Step -1
                            'sGraphic = Regex.Replace(GraphicNodes(c).Attributes("xlink:href").Value, "f(\d+)\.(\d+)", "fig$1_$2.tif", RegexOptions.IgnoreCase) ' 20-05-2022
                            sGraphic = $"{GraphicNodes(c).Attributes("xlink:href").Value.Replace("F", "fig")}.tif" ' 20-05-2022
                            GraphicNodes(c).InnerText = ""
                            If (Not String.IsNullOrEmpty(sGraphic)) Then
                                GraphicNodes(c).Attributes("xlink:href").Value = sGraphic
                            End If
                            If (AltNode IsNot Nothing) Then
                                FigureLst(g).InsertAfter(GraphicNodes(c), AltNode)
                            Else
                                If (CaptionNode IsNot Nothing) Then
                                    FigureLst(g).InsertAfter(GraphicNodes(c), CaptionNode)
                                End If
                            End If
                        Next
                    End If
            ...
            If ((UriList IsNot Nothing) AndAlso (UriList.Count > 0)) Then
                For Each UrlNode As XmlNode In UriList
                    ...
                    If (Not String.IsNullOrEmpty(UrlCont)) Then
                        'UrlCont = Regex.Replace(UrlCont, "(.*?)(http(?:s)?)", "$2", RegexOptions.Singleline Or RegexOptions.IgnoreCase)
                        UrlCont = UrlCont.Replace("httpshttp", "https")
                        UrlCont = UrlCont.Replace("httphttps", "http")
                    End If
                    If (Not String.IsNullOrEmpty(UrlCont.Trim()) And (Not UrlCont.Trim().StartsWith("http"))) Then
                        UrlCont = String.Format("http://{0}", UrlCont.Trim())
                    End If
                    UrlNode.Attributes("xlink:href").Value = UrlCont.Trim()
                    UrlNode.InnerXml = UrlNode.InnerXml.Trim()
                Next
            End If
        End If
        ...
        Dim AttribNodes As XmlNodeList = xmlFloat.SelectNodes("//disp-quote[following-sibling::*[1][local-name()='attrib']]")
        Dim MaxAtCnt As Int16 = 0
        If ((AttribNodes IsNot Nothing) AndAlso (AttribNodes.Count > 0)) Then
            MaxAtCnt = AttribNodes.Count * 2
            While ((AttribNodes IsNot Nothing) AndAlso (AttribNodes.Count > 0))
                If (MaxAtCnt = 0) Then
                    Exit While
                End If
                Dim AttribNode As XmlNode = AttribNodes(0).NextSibling
                If (AttribNode IsNot Nothing) Then
                    AttribNodes(0).AppendChild(AttribNode)
                End If
                AttribNodes = xmlFloat.SelectNodes("//disp-quote[following-sibling::*[1][local-name()='attrib']]")
                MaxAtCnt = MaxAtCnt - 1
            End While
        End If
        ...
        If ((MoreRowList IsNot Nothing) AndAlso (MoreRowList.Count > 0)) Then
            MdxCnt = MoreRowList.Count * 3
            While ((MoreRowList IsNot Nothing) AndAlso (MoreRowList.Count > 0))
                If (MdxCnt = 0) Then Exit While
                Try
                    Dim MorerowsAtt As XmlAttribute = xmlFloat.CreateNode(XmlNodeType.Attribute, "morerows", "")
                    MorerowsAtt.Value = Convert.ToInt32(MoreRowList(0).Attributes("rowspan").Value) - 1
                    MoreRowList(0).Attributes.Remove(MoreRowList(0).Attributes("rowspan"))
                    MoreRowList(0).Attributes.Append(MorerowsAtt)
                Catch ex As Exception
                End Try

                'If (MoreRowList(0).ParentNode IsNot Nothing) Then
                '    Try
                '        MoreRowList(0).ParentNode.InnerXml = MoreRowList(0).ParentNode.InnerXml.Replace(" rowspan=""", " morerows=""")
                '    Catch ex As Exception
                '    End Try
                'End If
                MoreRowList = xmlFloat.SelectNodes("//entry[@rowspan]")
                MdxCnt = MdxCnt - 1
            End While
        End If
        ...
        ''30-11-2021
        Dim LstAttb As XmlAttribute = Nothing
        Dim Lists As XmlNodeList = xmlFloat.SelectNodes("//list")
        If ((Lists IsNot Nothing) AndAlso (Lists.Count > 0)) Then
            Dim LabelTxt As String = String.Empty
            Dim IsRemoveToken As Boolean = False
            For l As Int16 = Lists.Count - 1 To 0 Step -1
                IsRemoveToken = False
                Try
                    Dim LabelNode As XmlNode = Lists(l).SelectSingleNode(".//token")
                    If (LabelNode Is Nothing) Then
                        GBL.DeantaBallon("Could not able to find the token node ", MessageType.MSGERROR)
                        Continue For
                    End If
                    Dim token As String = LabelNode.InnerText.Replace(".", "").Replace("(", "").Replace(")", "").Replace("[", "").Replace("]", "").Trim()

                    'If (Regex.IsMatch(token, "[0-9]+", RegexOptions.Singleline Or RegexOptions.IgnoreCase)) Then
                    '    LabelTxt = "order"
                    '    IsRemoveToken = True
                    'ElseIf (Regex.IsMatch(token, "(?i:[ivx]+)", RegexOptions.Singleline)) Then
                    '    LabelTxt = "roman-lower"
                    '    IsRemoveToken = True
                    'ElseIf (Regex.IsMatch(token, "(?i:[IVX]+)", RegexOptions.Singleline)) Then
                    '    LabelTxt = "roman-lower"
                    '    IsRemoveToken = True
                    'ElseIf (Regex.IsMatch(token, "(?i:[a-z]+)", RegexOptions.Singleline)) Then
                    '    LabelTxt = "alpha-lower"
                    '    IsRemoveToken = True
                    'ElseIf (Regex.IsMatch(token, "(?i:[A-Z]+)", RegexOptions.Singleline)) Then
                    '    LabelTxt = "alpha-upper"
                    '    IsRemoveToken = True
                    'Else
                    '    LabelTxt = "order"
                    '    IsRemoveToken = False
                    ...
                    Select Case token
                        ...
                        Case Else
                            ...
                            IsRemoveToken = False
                    End Select
                    If (Lists(l).Attributes("list-type") IsNot Nothing) Then
                        LstAttb = Lists(l).Attributes("list-type")
                        LstAttb.Value = LabelTxt
                    Else
                        LstAttb = xmlFloat.CreateNode(XmlNodeType.Attribute, "list-type", "")
                        LstAttb.Value = LabelTxt
                        Lists(l).Attributes.Append(LstAttb)
                    End If
                    If (IsRemoveToken) Then
                        Dim Tokens As XmlNodeList = Lists(l).SelectNodes(".//token")
                        If ((Tokens IsNot Nothing) AndAlso (Tokens.Count > 0)) Then
                            For t As Int16 = 0 To Tokens.Count - 1
                                If (Tokens(t).ParentNode IsNot Nothing) Then
                                    Tokens(t).ParentNode.RemoveChild(Tokens(t))
                                End If
                            Next
                        End If
                    End If
        ...
        For a As Integer = 0 To CommaList.Count - 1
            ...
            End Try
        Next
        Dim HolderNodes As XmlNodeList = xmlFloat.SelectNodes("//copyright/holder")
        Dim CopyText As String = String.Empty
        If (HolderNodes IsNot Nothing) Then
            For h As Int16 = 0 To HolderNodes.Count - 1
                Dim HolderNode As XmlNode = HolderNodes(h)
                CopyText = HolderNode.InnerText.Replace("&#x200A;", "")
                If ((Not String.IsNullOrEmpty(CopyText)) And (CopyText.Contains("00A9;"))) Then
                    ...
                    If (CopyMatch.Success) Then
                        If (HolderNode.ParentNode IsNot Nothing) Then
                            HolderNode.ParentNode.InnerXml = HolderNode.ParentNode.InnerXml.Replace(HolderNode.OuterXml, $"<permissions><copyright-statement>{HolderNode.InnerXml}</copyright-statement><copyright-year>{CopyMatch.Groups(1).Value}</copyright-year><copyright-holder>{CopyMatch.Groups(2).Value.Replace("&", "&amp;")}</copyright-holder></permissions>") 'HolderNode.OuterXml & String.Format("<copyright-year>{0}</copyright-year><copyright-holder>{1}</copyright-holder>", CopyMatch.Groups(1).Value, CopyMatch.Groups(2).Value.Replace("&", "&amp;")))
                        End If
                    End If
                End If
            Next
        ...
        '15-12-2021
        Try
            xmlFloat.MoveXMLNode("//publisher//permissions", "//book-meta/publisher", NodeMoveOption.MOVEAFTER)
        Catch ex As Exception
            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        End Try
        ...
        Dim IsbnNodes As XmlNodeList = xmlFloat.SelectNodes("//isbn[@pub-type='hbk']|//isbn[@pub-type='ebk']|//isbn[@pub-type='pbk']")
        If ((IsbnNodes IsNot Nothing) AndAlso (IsbnNodes.Count > 0)) Then
            For i As Integer = 0 To IsbnNodes.Count - 1
                IsbnNodes(i).InnerText = IsbnNodes(i).InnerText.Replace("ISBN: ", "").Replace("-", "").Replace("(hbk)", "").Replace("(ebk)", "").Replace("(pbk)", "").TrimEnd(" ")
            Next
        End If
        ...
        If ((BookPartNodes IsNot Nothing) AndAlso (BookPartNodes.Count > 0)) Then
            For k As Int16 = 0 To BookPartNodes.Count - 1
                Dim BodyNode As XmlNode = BookPartNodes(k).SelectSingleNode(".//body")
                If (BodyNode Is Nothing) Then
                    Dim BdNode As XmlNode = xmlFloat.CreateNode(XmlNodeType.Element, "body", "")
                    BookPartNodes(k).AppendChild(BdNode)
                End If
            Next
        End If
            ...
            If ((BookPartNodes IsNot Nothing) AndAlso (BookPartNodes.Count > 0)) Then
                While ((BookPartNodes IsNot Nothing) AndAlso (BookPartNodes.Count > 0))
                    ...
                    Try
                        ...
                        If (PartNode.NextSibling IsNot Nothing) Then
                            ChapNode = PartNode.NextSibling
                            While ((ChapNode IsNot Nothing) AndAlso (ChapNode.NodeType <> XmlNodeType.Element))
                                ChapNode = ChapNode.NextSibling
                                If ((ChapNode Is Nothing) AndAlso (ChapNode.NodeType = XmlNodeType.Element)) Then
                                    Exit While
                                End If
                            End While
                            'ChapNode = PartNode.ParentNode.SelectSingleNode(".//descendant::book-part")
                            ''If ((ChapNode IsNot Nothing) AndAlso (ChapNode.NodeType <> XmlNodeType.Element)) Then
                            ''    ChapNode = PartNode.NextSibling.NextSibling
                            ''End If
                        Else
                            Exit While
                        End If
                        While ((ChapNode IsNot Nothing) AndAlso (ChapNode.Attributes IsNot Nothing) AndAlso (ChapNode.Attributes.Count > 0) AndAlso (String.Compare(ChapNode.Attributes("book-part-type").Value, "chapter", True) = 0))

                            TmpChapNode = ChapNode.NextSibling
                            While ((TmpChapNode IsNot Nothing) AndAlso (TmpChapNode.NodeType <> XmlNodeType.Element))
                                TmpChapNode = TmpChapNode.NextSibling
                                If (TmpChapNode Is Nothing) Then
                                    Exit While
                                End If
                            End While
        ...
        'Comment as per dhasa request. '06-01-2022
        'Dim ContribNameLst As XmlNodeList = xmlFloat.SelectNodes("//contrib/name")
        'If ((ContribNameLst IsNot Nothing) AndAlso (ContribNameLst.Count > 0)) Then
        '    For c As Int16 = 0 To ContribNameLst.Count - 1
        '        Try
        '            Dim Bioid As String = GetBioID(ContribNameLst(c).InnerText, xmlFloat)
        '            If (Not String.IsNullOrEmpty(Bioid)) Then
        '                Dim BioNode As XmlNode = xmlFloat.CreateNode(XmlNodeType.Element, "xref", "")
        '                Dim refType As XmlAttribute = xmlFloat.CreateNode(XmlNodeType.Attribute, "ref-type", "")
        '                refType.Value = "bio"
        '                Dim RidAtt As XmlNode = xmlFloat.CreateNode(XmlNodeType.Attribute, "rid", "")
        '                RidAtt.Value = Bioid
        '                BioNode.Attributes.Append(refType)
        '                BioNode.Attributes.Append(RidAtt)
        '                ContribNameLst(c).ParentNode.AppendChild(BioNode)
        '            End If
        '        Catch ex As Exception
        '            GBL.DeantaBallon(ex.Message, MessageType.MSGERROR)
        '        End Try
        ...
        If ((BookPartList IsNot Nothing) AndAlso (BookPartList.Count > 0)) Then
            For pt As Integer = 0 To BookPartList.Count - 1
                ...
                If ((XrefPageList IsNot Nothing) AndAlso (XrefPageList.Count > 0) AndAlso (BodyNode IsNot Nothing)) Then
                    ...
                    End Try
                    Try
                        Dim iMax As Int16 = (From n In XrefPageList Select Convert.ToInt16(n.Attributes("id").Value.Replace("page_", ""))).Max
                        If (iMax > XrefPageList(XrefPageList.Count - 1).Attributes("id").Value.ToLower().Replace("page_", "")) Then
                            LastPageNode.InnerText = iMax
                        Else
                            LastPageNode.InnerText = XrefPageList(XrefPageList.Count - 1).Attributes("id").Value.ToLower().Replace("page_", "")
                        End If
        ...
        If (Not IsePubXML) Then
            ...
            Dim StartRng As String = String.Empty
            Dim EndRng As String = String.Empty
            Dim IndexPageRanges As XmlNodeList = xmlFloat.SelectNodes("//nav-pointer-group")
            If ((IndexPageRanges IsNot Nothing) AndAlso (IndexPageRanges.Count > 0)) Then
                For i As Int16 = 0 To IndexPageRanges.Count - 1
                    PageTmpNum = String.Empty
                    StartRng = String.Empty
                    EndRng = String.Empty
                    
                    Try
                        If (Not IndexPageRanges(i).InnerXml.Contains("&amp;#x2013;")) Then Continue For
                        Dim NavPointNodes As XmlNodeList = IndexPageRanges(i).SelectNodes(".//nav-pointer")
                        If ((NavPointNodes Is Nothing) OrElse (NavPointNodes.Count = 0)) Then Continue For
                        If (NavPointNodes.Count = 2) Then
                            StartRng = NavPointNodes(0).InnerText
                            EndRng = NavPointNodes(1).InnerText
                            If (StartRng.Length >= EndRng.Length) Then
                                Dim Diff As Integer = StartRng.Length - EndRng.Length
                                PageTmpNum = $"{(StartRng.Substring(0, Diff))}{EndRng}"
                                NavPointNodes(1).Attributes("rid").Value = $"page_{PageTmpNum}"
                            End If
                        ElseIf (NavPointNodes.Count > 2) Then
                            Dim IndexSepNodes As XmlNodeList = IndexPageRanges(i).SelectNodes(".//text()[.='&#x2013;']")
                            If ((IndexSepNodes IsNot Nothing) AndAlso (IndexSepNodes.Count > 0)) Then
                                For k As Int16 = 0 To IndexSepNodes.Count - 1
                                    StartRng = String.Empty
                                    EndRng = String.Empty
                                    Try
                                        StartRng = IndexSepNodes(k).PreviousSibling.InnerText
                                        EndRng = IndexSepNodes(k).NextSibling.InnerText
                                    Catch ex As Exception
                                        Continue For
                                    End Try
                                    If (String.IsNullOrEmpty(StartRng) Or String.IsNullOrEmpty(EndRng)) Then
                                        Continue For
                                    End If
                                    If (StartRng.Length >= EndRng.Length) Then
                                        Dim Diff As Integer = StartRng.Length - EndRng.Length
                                    ...
                                    If (Not String.IsNullOrEmpty(PageTmpNum)) Then
                                        Try
                                            IndexSepNodes(k).NextSibling.Attributes("rid").Value = $"page_{PageTmpNum}"
                                        Catch ex As Exception
                                        End Try
                                    End If
                                Next
        ...
        '18-09-2020
        GraphicNodes = xmlFloat.SelectNodes("//p[child::p/graphic]|//p[child::p/inline-graphic]")
        If ((GraphicNodes IsNot Nothing) AndAlso (GraphicNodes.Count > 0)) Then
            ...
            While ((GraphicNodes IsNot Nothing) AndAlso (GraphicNodes.Count > 0))
                ...
                Try
                    If (GraphicNodes(0).ParentNode IsNot Nothing) Then
                        If ((GraphicNodes(0).ParentNode.InnerXml.Contains("<p><p><graphic")) Or (GraphicNodes(0).ParentNode.InnerXml.Contains("<p><p><inline-graphic"))) Then
        ...
        BookParts = xmlFloat.SelectNodes("//book-part/book-part-id")
        If ((BookParts IsNot Nothing) AndAlso (BookParts.Count > 0)) Then
            For b As Int16 = 0 To BookParts.Count - 1
                Dim BkPartMeta As XmlNode = BookParts(b).ParentNode.SelectSingleNode(".//book-part-meta")
                If (BkPartMeta IsNot Nothing) Then
                    BkPartMeta.PrependChild(BookParts(b))
                End If
            Next
        End If

        Dim IbidList As New List(Of String)
        Dim IbidMat As String = String.Empty
        IbidList.AddRange(New String() {"Ibid.", "Ibidem", "op. cit.", "opus citatum", "opere citato", "ibid.", "ibidem", "Ibīdem", "ibīdem", "loc. cit.", "loco citato", "supra", "infra"})
        Dim Footnotes As XmlNodeList = xmlFloat.SelectNodes("//fn-group/fn[@id]/p")
        If ((Footnotes IsNot Nothing) AndAlso (Footnotes.Count > 0)) Then
            If ((Footnotes IsNot Nothing) AndAlso (Footnotes.Count > 0)) Then
                For f As Int16 = 0 To Footnotes.Count - 1
                    Try
                        IbidMat = String.Empty
                        If (String.IsNullOrEmpty(Footnotes(f).InnerText)) Then Continue For
                        'If (Not (From n In IbidList Where n.ToLower().Contains(Footnotes(f).InnerText.ToLower()) Select n).Any) Then Continue For
                        'If (Footnotes(f).InnerText.Contains("41 and 44; RESAS (n18)")) Then
                        '    MsgBox("OK")
                        'End If
                        Dim FtNode As XmlNode = Footnotes(f).ParentNode.PreviousSibling
                        If (FtNode Is Nothing) Then Continue For
                        If (Not FtNode.HasChildNodes) Then Continue For
                        Dim PaNode As XmlNode = FtNode.SelectSingleNode(".//p")
                        If (PaNode Is Nothing) Then Continue For
                        Dim IbidText As String = Regex.Match(PaNode.InnerXml, "&amp;#x201c;(((?!&amp;#x201d;).)*)&amp;#x201d;", RegexOptions.Singleline Or RegexOptions.IgnoreCase).Value
                        If (String.IsNullOrEmpty(IbidText)) Then Continue For
                        'If (Footnotes(f).ParentNode.OuterXml.Contains("ch5-fn315")) Then
                        '    MsgBox("ok")
                        'End If
                        IbidMat = (From n In IbidList Where (String.Compare(n, Footnotes(f).InnerText, False) = 0) Select n).FirstOrDefault
                        If (Not String.IsNullOrEmpty(IbidMat)) Then
                            Footnotes(f).InnerXml = Footnotes(f).InnerXml.Replace(IbidMat, IbidText)
                            Continue For
                        End If
                        IbidMat = (From n In IbidList Where Footnotes(f).InnerText.Contains(n) Select n).FirstOrDefault
                        If (Not String.IsNullOrEmpty(IbidMat)) Then
                            Footnotes(f).InnerXml = Footnotes(f).InnerXml.Replace(IbidMat, IbidText)
                            Continue For
                        End If
                        'IbidMat = (From n As String In IbidList Where n.Contains(IIf(Footnotes(f).InnerText.EndsWith("."), Footnotes(f).InnerText, $"{Footnotes(f).InnerText}.")) Select n).FirstOrDefault
                        'If (Not String.IsNullOrEmpty(IbidMat)) Then
                        '    Footnotes(f).InnerXml = Footnotes(f).InnerXml.Replace(IbidMat, IbidText)
                        '    Continue For
                        'End If
                        'IbidMat = (From n As String In IbidList Where n.Contains(IIf(Footnotes(f).InnerText.EndsWith(";"), Footnotes(f).InnerText, $"{Footnotes(f).InnerText.Replace(".", "")};")) Select n).FirstOrDefault
                        'If (Not String.IsNullOrEmpty(IbidMat)) Then
                        '    Footnotes(f).InnerXml = Footnotes(f).InnerXml.Replace(IbidMat, IbidText)
                        '    Continue For
                        'End If
                    ...
                    'If (String.Compare(Footnotes(f).InnerText, "Ibid.", True) = 0) Then
                    '    Footnotes(f).InnerXml = Footnotes(f).InnerXml.Replace("Ibid.", IbidText)
                    'ElseIf (Footnotes(f).InnerText.Contains("Ibid.")) Then
                    '    Footnotes(f).InnerXml = Footnotes(f).InnerXml.Replace("Ibid.", $"{IbidText}")
                    'ElseIf (Footnotes(f).InnerText.Contains("Ibid;")) Then
                    '    Footnotes(f).InnerXml = Footnotes(f).InnerXml.Replace("Ibid;", $"{IbidText}")
                    'End If
        ...
        xmlTemp = xmlTemp.Replace("<token>", "<!--<token>-->").Replace("</token>", "<!--</token>-->")
        ...
        '30-06-2021
        xmlTemp = Regex.Replace(xmlTemp, "(<fig[^>]*>)&#x2003;(<caption)", "$1$2", RegexOptions.Singleline Or RegexOptions.IgnoreCase)

        xmlTemp = xmlTemp.Replace("</table-wrap-foot><table-wrap-foot>", "")
        xmlTemp = Regex.Replace(xmlTemp, "(<imprint-text[^>]*>)(<imprint-text[^>]*>)", "$2", RegexOptions.Singleline Or RegexOptions.IgnoreCase)
        ...
        xmlTemp = xmlTemp.Replace(" book-part-type=""partt""", " book-part-type=""part""")
        xmlTemp = xmlTemp.Replace("&#x2019;", "&#x0027;")
        Return xmlTemp
    End Function

    Private Function GetBioID(ByVal AuthorName As String, ByVal xmlfloatDoc As XmlDocument) As String
        Dim BioList As XmlNodeList = xmlfloatDoc.SelectNodes("//bio[@id]/p/bold")
        If ((BioList IsNot Nothing) AndAlso (BioList.Count > 0)) Then
            For b As Int16 = 0 To BioList.Count - 1
                If (String.Compare(BioList(b).InnerText, AuthorName, True) = 0) Then
                    Return BioList(b).ParentNode.ParentNode.Attributes("id").Value
                End If
            Next
        End If
        Return String.Empty
    ...
    Private Function GetTableColumnCount(ByVal FirstRowNode As XmlNode) As Int16
        ...
        If ((FirstRowNode IsNot Nothing) AndAlso (FirstRowNode.ChildNodes IsNot Nothing) AndAlso (FirstRowNode.ChildNodes.Count > 0)) Then
            For Each RwNode As XmlNode In FirstRowNode.ChildNodes
                ...
                ElseIf ((RwNode.Attributes("namest") IsNot Nothing) AndAlso (RwNode.Attributes("nameend") IsNot Nothing)) Then
                    iColCount = iColCount + (RwNode.Attributes("nameend").Value - RwNode.Attributes("namest").Value) + 1
                Else
    ...
    Private Function ImprintPro(m As Match)
        ...
        ElseIf (sResult.ToLower.Contains("all rights reserved") Or sResult.ToLower.Contains("reasonable efforts")) Then
        ...
        ElseIf sResult.ToLower.Contains("crc Press") Then
            sResult = Regex.Replace(sResult, "<imprint-text>", "<imprint-text type=""ImprintStatement"">", RegexOptions.IgnoreCase Or RegexOptions.Singleline)
    ...
    Private Function UpdatePro(ByVal sChapterTxt As String, ByVal bxslExecution As Boolean) As String
        ...
        iVal = 0
        sChapterTxt = Regex.Replace(sChapterTxt, "(<abbreviation([^><]+)?)>", AddressOf IDGen, RegexOptions.IgnoreCase Or RegexOptions.Singleline)
    ...
    Private Function PrefacePro(m As Match) As String
    ...
    Private Function ChapterProEpub(m As Match)
        ...
        iVal = iVal + 1
        If (TmpStr.Contains("xml:id")) Then
            TmpStr = Regex.Replace(TmpStr, " xml:id=""([^""])+""", " xml:id=""b-" & sISBN & "-chapter" & iVal & """", RegexOptions.IgnoreCase Or RegexOptions.Singleline)
        Else
            TmpStr = Regex.Replace(TmpStr, ">", " xml:id=""b-" & sISBN & "-chapter" & iVal & """>", RegexOptions.IgnoreCase Or RegexOptions.Singleline)
        End If

        Return TmpStr
    End Function
    Private Function old_ChapterProEpub(m As Match)
        If m.Value.ToString.Contains("/>") OrElse m.Value.ToString.Contains("chapter-title") Then Return m.Value.ToString
        Dim TmpStr As String = m.Value
        Dim sInput As String = m.Groups(1).Value.ToString
        ...
        TmpStr = Regex.Replace(TmpStr, " xml:id=""([^""])+""", "", RegexOptions.IgnoreCase Or RegexOptions.Singleline)
    ...
    Private Function IDGen(m As Match)
        ...
        ElseIf m.Groups(1).Value.ToString.Contains("<index") Then
            sResults = sInput & " xml:id=" & Chr(34) & "b-" & sISBN & "-index" & Chr(34) & ">"
        ...
        ElseIf m.Groups(1).Value.ToString.Contains("<abbreviation") Then
            sResults = sInput & " xml:id=" & Chr(34) & "b-" & sISBN & "-abbrev" & Chr(34) & ">"
        ...
        ElseIf m.Groups(1).Value.ToString.Contains("<toc") Then
            If ((Not m.Groups(1).Value.ToString.Contains("content-type")) And (Not m.Value.ToString.Contains("<toc-div")) And (Not m.Value.ToString.Contains("<toc-entry")) And (Not m.Value.ToString.Contains("<toc-title"))) Then
    ...
    Private Function EpubCleanup(sContent As String)
        ...
        sContent = Regex.Replace(sContent, "(<chapter[^><]+>)((?:(?!<para|chapter([^><]+)?>).)+)(<para([^><]+)?>)", AddressOf ChapterInfo, RegexOptions.IgnoreCase Or RegexOptions.Singleline)
        ...
        sContent = Regex.Replace(sContent, "(<chapter[^><]+>)((?(?!<para|chapter([^><]+)?>).)+)(</chapter([^><]+)?>)", AddressOf ChapterInfo, RegexOptions.IgnoreCase Or RegexOptions.Singleline)
        sContent = Regex.Replace(sContent, "(<chapter[^><]+>)((?:(?!<para|chapter([^><]+)?>).)+)(<bibliography([^><]+)?>)", AddressOf ChapterInfo, RegexOptions.IgnoreCase Or RegexOptions.Singleline)
        sContent = Regex.Replace(sContent, "(<abbreviation[^><]+>)((?:(?!<glosslist([^><]+)?>).)+)(<glosslist([^><]+)?>)", AddressOf ChapterInfo, RegexOptions.IgnoreCase Or RegexOptions.Singleline)
        ...
        'sContent = Regex.Replace(sContent, "<info([^><]+)?>(((?!</info>).)+)</info>", AddressOf InfoRepeat, RegexOptions.IgnoreCase Or RegexOptions.Singleline)  ''29-01-2021
        ...
        If (IsePubXML) Then
            ...
            EpubMgr.DocType = Me.DocType
            EpubMgr.OrgDocType = Me.OrgDocType
            EpubMgr.IsBookEndNote = IsBookEndNote
            Try
    ...
    Private Function ChapterInfo(m As Match)
        ...
        LofList.AddRange(New String() {"list of figures", "list of figure", "figures", "figure", "list of illustrations", "list of illustration", "illustrations", "illustration"})
        LotList.AddRange(New String() {"list of tables", "list of table", "tables", "table"})
        If (sResult.Contains("<bibliography ")) Then
            sResult = Regex.Replace(sResult, "(xml:id=""[^""]+\-)(chapter(\d+)?)""", "$1ref""", RegexOptions.IgnoreCase)
            Return sResult
        End If
        ...
        Dim TitleText As String = Regex.Match(sResult, "<title([^><]+)?>((?:(?!</title>).)+)</title>", RegexOptions.IgnoreCase Or RegexOptions.Singleline).Groups(2).Value.ToString.ToLower()
        TitleText = Regex.Replace(TitleText, "<a [^>]*/>", "", RegexOptions.Singleline Or RegexOptions.IgnoreCase)

        If (TitleText.Contains("introduction")) Then
            If (Not m.Value.ToString.Contains("<label")) Then
            ...
            End If
        ElseIf TitleText.Contains("bibliography") Then
            sResult = Regex.Replace(sResult, "(xml:id=""[^""]+\-)(chapter(\d+)?)""", "$1bib""", RegexOptions.IgnoreCase Or RegexOptions.Singleline)

            sResult = Regex.Replace(sResult, " label=""[^""]+""", "", RegexOptions.IgnoreCase Or RegexOptions.Singleline)

            ElseIf TitleText.Contains("references") Then
            sResult = Regex.Replace(sResult, "(xml:id=""[^""]+\-)(chapter(\d+)?)""", "$1bib""", RegexOptions.IgnoreCase Or RegexOptions.Singleline)

            sResult = Regex.Replace(sResult, " label=""[^""]+""", "", RegexOptions.IgnoreCase Or RegexOptions.Singleline)

            ElseIf TitleText.Contains("about the contributors") Then
            sResult = Regex.Replace(sResult, "(xml:id=""[^""]+\-)(chapter(\d+)?)""", "$1abocon""", RegexOptions.IgnoreCase Or RegexOptions.Singleline)

            sResult = Regex.Replace(sResult, " label=""[^""]+""", "", RegexOptions.IgnoreCase Or RegexOptions.Singleline)

            ElseIf TitleText.Contains("acronyms") Then
            sResult = Regex.Replace(sResult, "(xml:id=""[^""]+\-)(chapter(\d+)?)""", "$1acro""", RegexOptions.IgnoreCase Or RegexOptions.Singleline)

            sResult = Regex.Replace(sResult, " label=""[^""]+""", "", RegexOptions.IgnoreCase Or RegexOptions.Singleline)

            ElseIf TitleText.Contains("credits") Then
            sResult = Regex.Replace(sResult, "(xml:id=""[^""]+\-)(chapter(\d+)?)""", "$1cred""", RegexOptions.IgnoreCase Or RegexOptions.Singleline)

            sResult = Regex.Replace(sResult, " label=""[^""]+""", "", RegexOptions.IgnoreCase Or RegexOptions.Singleline)

            ElseIf TitleText.Contains("about the author") Then
            sResult = Regex.Replace(sResult, "(xml:id=""[^""]+\-)(chapter(\d+)?)""", "$1about""", RegexOptions.IgnoreCase Or RegexOptions.Singleline)

            sResult = Regex.Replace(sResult, " label=""[^""]+""", "", RegexOptions.IgnoreCase Or RegexOptions.Singleline)

            ElseIf TitleText.Contains("notes") Then

            sResult = Regex.Replace(sResult, "(xml:id=""[^""]+\-)(chapter(\d+)?)""", "$1note""", RegexOptions.IgnoreCase Or RegexOptions.Singleline)
                sResult = Regex.Replace(sResult, "<chapter[^>]*>", Function(mt As Match)
                                                                       Return Regex.Replace(mt.Value, " label=""[^""]+""", "", RegexOptions.IgnoreCase Or RegexOptions.Singleline)
                                                                   End Function, RegexOptions.IgnoreCase Or RegexOptions.Singleline)

            ElseIf (From n In LofList Where String.Compare(n, TitleText, True) = 0).Any Then

            sResult = Regex.Replace(sResult, "(xml:id=""[^""]+\-)(chapter(\d+)?)""", "$1fig""", RegexOptions.IgnoreCase Or RegexOptions.Singleline)

            ElseIf (From n In LotList Where String.Compare(n, TitleText, True) = 0).Any Then
            sResult = Regex.Replace(sResult, "(xml:id=""[^""]+\-)(chapter(\d+)?)""", "$1tab""", RegexOptions.IgnoreCase Or RegexOptions.Singleline)

        ElseIf (TitleText.Contains("conclusion") Or TitleText.Contains("conclusions")) Then

            If (Not m.Value.ToString.Contains("<label")) Then
                    sResult = Regex.Replace(sResult, "(xml:id=""[^""]+\-)(chapter(\d+)?)""", "$1conc""", RegexOptions.IgnoreCase Or RegexOptions.Singleline)
                    sResult = Regex.Replace(sResult, " label=""[^""]+""", "", RegexOptions.Singleline Or RegexOptions.IgnoreCase)
                End If

            ElseIf TitleText.Contains("appendices") Then
            sResult = Regex.Replace(sResult, "(xml:id=""[^""]+\-)(chapter(\d+)?)""", "$1appendix0""", RegexOptions.IgnoreCase)
            sResult = Regex.Replace(sResult, "<remark([^><]+)?>((?:(?!</remark>).)+)</remark>", "", RegexOptions.Singleline Or RegexOptions.IgnoreCase)
        ElseIf TitleText.Contains("abbreviation") Then
            sResult = Regex.Replace(sResult, "(xml:id=""[^""]+\-)(chapter(\d+)?)""", "$1abbrev""", RegexOptions.IgnoreCase)
        End If
        ...
        If AppMatch.Success Then
            Dim AppNumMatch As Match = Regex.Match(AppMatch.Groups(2).Value, "appendix ([0-9-a-z]+)", RegexOptions.IgnoreCase Or RegexOptions.Singleline)
            If (AppNumMatch.Success) Then
                ...
                sResult = Regex.Replace(sResult, "<remark([^><]+)?>((?:(?!</remark>).)+)</remark>", "", RegexOptions.Singleline Or RegexOptions.IgnoreCase)
            Else
                AppNumMatch = Regex.Match(AppMatch.Groups(2).Value, "appendix", RegexOptions.IgnoreCase Or RegexOptions.Singleline)
                If (AppNumMatch.Success) Then
                    sResult = Regex.Replace(sResult, "(xml:id=""[^""]+\-)(chapter(\d+)?)"">", "$1appendix" & AppNumMatch.Groups(1).Value & """>", RegexOptions.IgnoreCase)
                    sResult = Regex.Replace(sResult, "<remark([^><]+)?>((?:(?!</remark>).)+)</remark>", "", RegexOptions.Singleline Or RegexOptions.IgnoreCase)
                End If
            End If
        End If
        Dim st As Match = Regex.Match(m.Value.ToString, "<label>((?:(?!</label>).)+)</label>", RegexOptions.IgnoreCase)
        If st.Success Then
                sResult = Regex.Replace(sResult, "(xml:id=""[^""]+\-)(chapter(\d+)?)""", Function(mt As Match)
                                                                                             Return mt.Value.Replace("-chapter", "-c")
