...
.Copy-TXI1 {
		...
		<xsl:result-document href="{translate($bid,'-','')}/OEBPS/xhtml/cover.xhtml" method="xml" indent="yes">
			...
			<html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops" xml:lang="en" lang="en">
				...
				<body epub:type="cover">
					<img  id="cover-page" role="doc-cover" src="../images/cover.jpg" alt=""/>
				</body>
			</html>
		</xsl:result-document>
			...
			<html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops" xml:lang="en" lang="en">
				...
				<body epub:type="frontmatter">
					...
					<xsl:for-each select="//db:chapter[not(@role='notes' or contains(@xml:id, 'note'))]|//db:appendix|//db:part[child::db:label]|//db:index">
						...
						<p class="{$classattrib}">
							<a href="{substring-after(substring-after(@xml:id,'-'),'-')}.xhtml#{substring-after(substring-after(@xml:id,'-'),'-')}" id="re_{substring-after(substring-after(@xml:id,'-'),'-')}">
								<xsl:if test="child::db:label">
									<span class="cs_bl"><xsl:value-of select="@label"/><xsl:value-of select="child::db:label"/></span>
									<xsl:text> </xsl:text>
								</xsl:if>
								<xsl:if test="@label">
									<xsl:value-of select="@label"></xsl:value-of><xsl:text disable-output-escaping="yes"> </xsl:text>	
								</xsl:if>
								<xsl:apply-templates select="db:info/db:title" mode="toc"/>
								<xsl:if test="db:info/db:subtitle">
									<xsl:apply-templates select="db:info/db:subtitle" mode="toc"/>
			...
			<a id="{substring-after(substring-after((ancestor::db:chapter|ancestor::db:abbreviation|ancestor::db:appendix)/@xml:id,'-'),'-')}" href="contents.xhtml#re_{substring-after(substring-after((ancestor::db:chapter|ancestor::db:abbreviation|ancestor::db:appendix)/@xml:id,'-'),'-')}">
				<xsl:choose>
					<xsl:when test=".//parent::db:info/parent::db:chapter/@label">
						<span epub:type="ordinal">Chapter <xsl:value-of select=".//parent::db:info/parent::db:chapter/@label"/></span><br/><xsl:text disable-output-escaping="yes"> </xsl:text><xsl:apply-templates select="node() except (processing-instruction('page'),db:footnote,db:subtitle)"/>	
					</xsl:when>
					<xsl:otherwise>
	...
	<xsl:template match="db:para" mode="#all">
		...
		<xsl:choose>
			...
			<xsl:when test="./preceding-sibling::*[1][local-name()='dialogue' or local-name()='poem']">
				<p>
					<xsl:attribute name="class">TXT</xsl:attribute>
					<xsl:apply-templates/>
				</p>
	...
	<xsl:template match="db:footnote[@role='end-bk1-note']">
		<sup>
			<a href="{concat('notes.xhtml#',concat(concat('notes_',substring-after(substring-after(ancestor::db:chapter/@xml:id,'-'),'-')),'_f-'),@label)}" id="{concat(concat(substring-after(substring-after(ancestor::db:chapter/@xml:id,'-'),'-'),'_f-'),@label)}">
				<xsl:value-of select="@label"/>
			</a>
		</sup>
		<xsl:apply-templates select="node() except db:superscript"/>
	</xsl:template>
		...
		<xsl:result-document href="{translate($bid,'-','')}/OEBPS/xhtml/title.xhtml" method="xml" use-character-maps="hex" indent="yes">
			...
			<html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops" xml:lang="en" lang="en">
				...
				<body epub:type="frontmatter">
					...
					<p class="PUB"><img src="../images/logo.jpg" alt=""/></p>
					</section>
				</body>
			</html>
		</xsl:result-document>
	...
	<xsl:template match="db:copyright">
		<xsl:result-document href="{translate($bid,'-','')}/OEBPS/xhtml/copyright.xhtml" method="xml" use-character-maps="hex" indent="yes">
			...
			<html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops" xml:lang="en" lang="en">
				...
				<body>
					<!--<xsl:apply-templates select="./parent::db:info/db:bibliomisc[@role='imprint']|./child::db:holder|following-sibling::db:legalnotice/db:para|following-sibling::db:biblioset[@role='isbns']/db:biblioid|preceding-sibling::db:biblioset|preceding-sibling::db:edition|preceding-sibling::db:pubdate"/>-->
					<xsl:apply-templates select="preceding-sibling::db:bibliomisc|./parent::db:info/db:biblioset/db:phrase[1]|./parent::db:info/db:biblioset[@role='publisher']|./child::db:holder|following-sibling::db:legalnotice/db:para|./parent::db:info/db:biblioset[@role='isbns']|following-sibling::db:colophon/db:para"/>
				</body>
			</html>
		</xsl:result-document>
	</xsl:template>
	...
	<xsl:template match="db:superscript[not (parent::db:footnote)]">
...
<!--	<xsl:template match="db:link[@role='bibr']">
		...
		<a>
			<xsl:choose>
				...
				<xsl:when test="./@linkend">
					<xsl:choose>
						<xsl:when test="count(//db:book/db:bibliography)=1">
							<xsl:attribute name="href">bibliography.xhtml#<xsl:value-of select="./@linkend"></xsl:value-of></xsl:attribute>
						</xsl:when>
						<xsl:otherwise>
							<xsl:attribute name="href"><xsl:value-of select="./@linkend"></xsl:value-of></xsl:attribute>		
						</xsl:otherwise>
					</xsl:choose>
	...
	<xsl:template match="db:footnote[@role='end-bk-note']/node()" mode="note">
		...
		<p class="{$ftstyle}">
			<xsl:if test="not(preceding-sibling::db:para)">
				<a href="{concat(concat(./parent::db:footnote/@chapxmlid,'.xhtml#'),./parent::db:footnote/@chapxmlid,'_f-',parent::db:footnote/@label)}" id="{concat(concat('notes_',./parent::db:footnote/@chapxmlid),'_f-',parent::db:footnote/@label)}">
				<xsl:value-of select="./ancestor::db:footnote/@label"></xsl:value-of>	
				</a>
			</xsl:if>
			<xsl:apply-templates/>
		</p>
			...
			<xsl:if test="not(preceding-sibling::db:para)">
				<xsl:choose>
					<xsl:when test="(ancestor::db:chapter|ancestor::db:appendix|ancestor::db:preface)/@xml:id">
						<a href="{concat(concat(substring-after(substring-after((ancestor::db:chapter|ancestor::db:part[child::db:label])/@xml:id,'-'),'-'),'.xhtml'),'#fnt-',parent::db:footnote/@id)}" id="{concat('fn-',parent::db:footnote/@id)}">
	...
	<xsl:template match="db:bibliomixed">
		<li epub:type="biblioentry" id="{./@xml:id}" class="Biblio">
			<xsl:apply-templates/>
		</li>
	</xsl:template>
		...
		<xsl:variable name="lines">
			<xsl:choose>
				...
				<xsl:when test="./ancestor::db:dialogue">
					<xsl:choose>
						<xsl:when test="./count(parent::db:speech/preceding-sibling::db:speech) = 0">EXT-F1</xsl:when>
						<xsl:when test="(./count(parent::db:speech/preceding-sibling::db:speech) &gt;= 1) and ((./count(parent::db:speech/parent::db:dialogue/child::db:speech))-1)!=(./count(parent::db:speech/preceding-sibling::db:speech))">EXT-M1</xsl:when>
						<xsl:otherwise>EXT-L1</xsl:otherwise>	
					</xsl:choose>
				</xsl:when>
			</xsl:choose>
