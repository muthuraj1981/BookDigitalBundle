...
namespace ConvertEquations
{
	...
	class Program
	{
        ...
        public bool ConvertMML2EPS(String OutputPath)
        {
            ...
            {
                ce.Convert(new EquationInputClipboardText(ClipboardFormats.cfMML),
            new EquationOutputFileEPS(OutputPath));
                return true;
            }
            catch (Exception ex)
            {
		...
		public void Temp(ConvertEquation ce)
		{
            ...
            XmlNodeList MathNodeList = xmlDoc.SelectNodes("//equation");
            for(Int32 m=0;m<MathNodeList.Count-1;m++) {
                if (MathNodeList[m].NodeType != XmlNodeType.Element) { continue; }
                try
                {
                    String MathContent = MathNodeList[m].InnerXml;
                    XmlElement MathNode = ((XmlElement)MathNodeList[m].ChildNodes[0]);
                    String MathFilename = MathNode.GetAttribute("id");
                    Clipboard.SetText(MathContent);
                    ce.Convert(new EquationInputClipboardText(ClipboardFormats.cfMML),
                    new EquationOutputFileEPS(@"D:\Support\MathML_EPS\input\Data\" + MathFilename + ".eps"));
                    Clipboard.Clear();
                }
                catch (Exception ex)
                {
            ...
            //Clipboard.Clear();
            //Clipboard.SetText(File.ReadAllText(@"D:\Support\MathML_EPS\input\test.mml"));
            //ce.Convert(new EquationInputClipboardText(ClipboardFormats.cfMML),
            //       new EquationOutputFileEPS(@"D:\Support\MathML_EPS\input\test3.eps"));
            //Console.ReadKey();
            
                
            //ce.Convert(new EquationInputFileText(@"D:\Support\MathML_EPS\input\temp.mml", "MathML2 (m namespace).tdl"),
            //    new EquationOutputFileEPS(@"D:\Support\MathML_EPS\input\temp.eps"));

            //////working
            ////ce.Convert(new EquationInputFileEPS(@"D:\Support\MathML_EPS\input\TNF-CH005_eqn_0214.eps"),
            ////    new EquationOutputFileText(@"D:\Support\MathML_EPS\input\temp.txt", "MathML2 (m namespace).tdl"));
